...
private:
    ...
    cbHeaderEnd(const char *, size_t, size_t, void *userData) {
        MultipartReader *self = static_cast<MultipartReader *>(userData);
        self->currentHeaders.headers()[self->currentHeaderName].push_back(std::move(self->currentHeaderValue));
        self->currentHeaderName.clear();
        self->currentHeaderValue.clear();
    }
...
Body::get_compressor_from_header(const std::string &encoding) {
    ...
    if (has_identity)
        return nullptr; // Explicit identity means no compression
    if (only_chunked_or_empty && tokens.empty())
        return nullptr; // Empty encoding means no compression
    if (only_chunked_or_empty && !tokens.empty())
        return nullptr;
