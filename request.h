...
public:
     ...
     */
    Request(qb::http::method m, qb::io::uri u, qb::icase_unordered_map<std::vector<std::string>> h = {}, Body b = {})
     ...
     * @param b The body content for the request (moved). Defaults to an empty body.
     */
    explicit Request(qb::io::uri u, qb::icase_unordered_map<std::vector<std::string>> h = {}, Body b = {})
        : internal::MessageBase(std::move(h), std::move(b))
        , _method(Method::GET)
        , _uri(std::move(u)) {}
    ...
    cookie(const std::string &name) const noexcept {
     ...
     */
    [[nodiscard]] bool
    has_cookie(const std::string &name) const noexcept {
     ...
     */
    Request &
    with_header(std::string name, std::string value) {
     ...
     */
    Request &
    with_headers(qb::icase_unordered_map<std::vector<std::string>> h) {
     ...
     */
    Request &
    with_cookies(const CookieJar &cookies) {
        _cookies = cookies;
        return *this;
    }
