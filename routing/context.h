...
private:
    ...
    std::optional<std::string> _cancellation_reason_internal; ///< Stores the reason if context processing is cancelled.

    std::vector<std::shared_ptr<IAsyncTask<SessionType>>> _task_chain; ///< The current chain of tasks to be executed.
    ...
    bool _is_completed_internally = false; ///< Flag: `true` if `complete()` has been called with a terminal result (COMPLETE, CANCELLED, ERROR leading to finalization).
                    ...
                    if (_current_processing_phase == ProcessingPhase::ERROR_CHAIN) { 
                        ...
                        if (router_core_shared && router_core_shared->is_error_chain_set()) {
                            ...
                            if (!error_chain_tasks_list.empty()) {
                                ...
                                std::vector<std::shared_ptr<IAsyncTask<SessionType>>> error_chain_vec(error_chain_tasks_list.begin(), error_chain_tasks_list.end());
