...
class Router {
private:
    std::shared_ptr<RouterCore<SessionType>> _router_core; ///< The core routing engine responsible for matching and execution.
    std::shared_ptr<RouteGroup<SessionType>> _root_group;  ///< A default root group attached to the router for convenience.
...
public:
    ...
    template <typename CustomRouteType, typename... Args, typename = std::enable_if_t<std::is_base_of_v<ICustomRoute<SessionType>, CustomRouteType>>>
