    ...
    void cancel() override {
        ...
        if (_middleware) {
            try {
                ...
                std::cerr << "MiddlewareTask [" << name() << "]: Exception during _middleware->cancel(): " << e.what() << std::endl;
            } catch (...) {
                std::cerr << "MiddlewareTask [" << name() << "]: Unknown exception during _middleware->cancel()." << std::endl;
            }
        }
    }
...
public:
    ...
    void process(std::shared_ptr<Context<SessionType>> ctx) override {
        _handler_fn(ctx, [ctx_capture = ctx, middleware_name = _name]() { // Pass the 'next' callback
    ...
    void cancel() override { /* Optional: Implement cancellation logic if needed */ }
};
