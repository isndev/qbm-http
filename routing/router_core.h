...
private:
     ...
     */
    void
    compile_default_not_found_handler(const std::vector<std::shared_ptr<IAsyncTask<SessionType>>> &global_prefix_tasks) {
    ...
    compile_default_method_not_allowed_handler(const std::vector<std::shared_ptr<IAsyncTask<SessionType>>> &global_prefix_tasks) {
        ...
        _compiled_method_not_allowed_tasks.insert(_compiled_method_not_allowed_tasks.end(), global_prefix_tasks.begin(),
                                                  global_prefix_tasks.end());
        _compiled_method_not_allowed_tasks.push_back(std::make_shared<RouteLambdaTask<SessionType>>(
...
public:
     ...
     */
    void
    add_handler_node(std::shared_ptr<IHandlerNode<SessionType>> node) {
        if (node) {
     ...
     */
    void
    register_compiled_route(const std::string &full_path, qb::http::method method_val,
     ...
     */
    void
    set_not_found_handler(RouteHandlerFn<SessionType> handler_fn) {
     ...
     */
    void
    set_error_task_chain(std::vector<std::shared_ptr<IAsyncTask<SessionType>>> error_chain) {
     ...
     */
    [[nodiscard]] bool
    is_error_chain_set() const noexcept {
    ...
    route_request(std::shared_ptr<SessionType> session_ptr, qb::http::Request request_obj) {
        ...
        auto ctx = std::make_shared<Context<SessionType>>(std::move(request_obj), std::move(response_prototype), std::move(session_ptr),
        ...
        if (request_path_sv.length() > MAX_PATH_LENGTH) {
            LOG_HTTP_WARN("Path length exceeds maximum (" << request_path_sv.length() << " > " << MAX_PATH_LENGTH
                                                          << "): " << request_path_sv.substr(0, 100) << "...");
        ...
        } else if (auto allowed_info = _radix_tree.allowed_methods(request_path_sv); allowed_info && !allowed_info->methods.empty()) {
