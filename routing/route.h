...
namespace qb::http {
    ...
    public:
         ...
         */
        Route(std::string path_segment, qb::http::method http_method,
              std::shared_ptr<ICustomRoute<SessionType> > custom_route_ptr)
            : IHandlerNode<SessionType>(std::move(path_segment))
              , _http_method(http_method)
                ...
                if constexpr (std::is_same_v<T, RouteHandlerFn<SessionType> >) {
                    if (arg) {
                        ...
                        final_tasks.push_back(
                            std::make_shared<RouteLambdaTask<SessionType> >(arg, "HandlerFor:" + full_route_path));
                    }
