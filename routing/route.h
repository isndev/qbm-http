...
public:
    ...
    cancel() noexcept override {
        /* No-op by default for simple lambda handlers */
    }
...
private:
    ...
    using RouteLogicVariant = std::variant<RouteHandlerFn<SessionType>, std::shared_ptr<ICustomRoute<SessionType>>>;
...
public:
    ...
    Route(std::string path_segment, qb::http::method http_method, std::shared_ptr<ICustomRoute<SessionType>> custom_route_ptr)
        ...
        if (!logic_ptr || !(*logic_ptr)) {
            // Check if variant holds the ptr and ptr is not null
    ...
    [[nodiscard]] qb::http::method
    get_http_method() const noexcept {
        return _http_method;
    }
     ...
     */
    void
    compile_tasks_and_register(RouterCore<SessionType> &router_core, const std::string &current_built_path,
        ...
        std::visit(
            [&final_tasks, &full_route_path](auto &&arg) {
                using T = std::decay_t<decltype(arg)>;
                if constexpr (std::is_same_v<T, RouteHandlerFn<SessionType>>) {
                    if (arg) {
                        // arg is RouteHandlerFn<SessionType>
                    ...
                    if (arg) {
                        // arg is std::shared_ptr<ICustomRoute<SessionType>>
