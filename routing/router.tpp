...
// Method to enable CORS with given options
template <typename Session, typename String>
Router<Session, String> &
Router<Session, String>::enable_cors(const CorsOptions &options) {
    // Store the options
    ...
    use([this](Context &ctx) {
        ...
        if (_cors_options->allow_all_origins() &&
            _cors_options->match_strategy() !=
                CorsOptions::OriginMatchStrategy::Function &&
            ...
            if (_cors_options->allow_credentials() ==
        ...
        if (ctx.method() == HTTP_OPTIONS) {
            ...
            if (!request_method.empty()) {
                ...
                ctx.response.add_header("Access-Control-Max-Age",
                                        std::to_string(_cors_options->max_age()));
...
// Method to enable CORS with regex patterns
template <typename Session, typename String>
Router<Session, String> &
Router<Session, String>::enable_cors_with_patterns(
    const std::vector<std::string> &regex_patterns,
    const std::vector<std::string> &methods, const std::vector<std::string> &headers,
    bool allow_credentials) {
    return enable_cors(CorsOptions()
...
// Method to enable CORS in development mode (permissive)
template <typename Session, typename String>
Router<Session, String> &
Router<Session, String>::enable_dev_cors() {
