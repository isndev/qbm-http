...
namespace qb::http {
    ...
    struct Slot {
        ...
        consteval explicit Slot(std::string_view n) noexcept : name(n) {}
        consteval explicit Slot(const char* n) noexcept : name(n) {}

        /// Explicit access to the slot's value type &mdash; useful for
        /// template deduction in third-party code without pulling in `Slot`
        /// itself.
        [[nodiscard]] constexpr std::string_view key() const noexcept { return name; }
    };
    ...
    namespace detail {
        ...
        [[nodiscard]] inline std::string slot_key_to_string(std::string_view name) {
