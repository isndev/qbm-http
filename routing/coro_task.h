    ...
    namespace detail {
        ...
        make_coro_task_runner(CoroFn handler, DefaultCoroOutcome default_outcome, const char* kind) {
            return [handler   = std::move(handler),
                    default_outcome,
                    kind](std::shared_ptr<Context<SessionType>> ctx) {
                if (!ctx) {
                ...
                auto spawn = [handler, default_outcome, kind, ctx]() mutable
                    ...
                    try {
                        ...
                        LOG_HTTP_ERROR("Coro " << kind << " exception: method="
                                       << std::to_string(ctx->request().method())
                        ...
                        LOG_HTTP_ERROR("Coro " << kind << " unknown exception: method="
                                       << std::to_string(ctx->request().method())
                                       << " path=" << ctx->request().uri().path());
                        if (!ctx->is_completed() && !ctx->is_cancelled()) {
                            ctx->response().status() = qb::http::status::INTERNAL_SERVER_ERROR;
        ...
        wrap_coro_route_handler(CoroFn&& handler) {
            return make_coro_task_runner<SessionType>(
                std::forward<CoroFn>(handler),
                DefaultCoroOutcome::COMPLETE_RESPONSE,
                "route");
        }
        ...
        wrap_coro_middleware_handler(CoroFn&& handler) {
            auto runner = make_coro_task_runner<SessionType>(
                std::forward<CoroFn>(handler),
                DefaultCoroOutcome::CONTINUE_CHAIN,
                "middleware");
