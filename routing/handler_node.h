 ...
 */
inline std::string
normalize_path_segment(const std::string &segment) {
    if (segment.empty()) {
        return "";
    }
 ...
 */
inline std::string
join_paths(const std::string &parent, const std::string &segment) {
...
public:
     ...
     */
    void
    set_parent(std::weak_ptr<IHandlerNode<Session>> parent) noexcept {
        _parent = parent;
    }
     ...
     */
    [[nodiscard]] std::shared_ptr<IHandlerNode<Session>>
    ...
    get_path_segment() const noexcept {
     ...
     */
    void
    add_middleware(std::shared_ptr<IAsyncTask<Session>> middleware_task) {
        if (middleware_task) {
     ...
     */
    virtual void compile_tasks_and_register(RouterCore<Session> &router_core, const std::string &current_built_path,
...
protected:
    ...
    build_full_path(const std::string &parent_full_path) const {
