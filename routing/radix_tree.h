...
namespace qb::http {
    ...
    struct MatchedRouteInfo {
        ...
        MatchedRouteInfo(PathParameters params,
                         std::optional<std::shared_ptr<const std::vector<std::shared_ptr<IAsyncTask<SessionType>>>>> tasks)
...
private:
    ...
    struct Node {
        ...
        std::string_view segment_match;   ///< For STATIC nodes: the exact segment string. For PARAMETER/WILDCARD nodes: the name of the parameter/wildcard (e.g., "id", "filepath").
    ...
    [[nodiscard]] static std::vector<std::string_view> split_path_to_segments(std::string_view path_sv) noexcept {
        ...
        for (size_t i = start; i < n; ++i) {
            if (path_sv[i] == '/') {
                if (i > start) { // Avoid empty segment from consecutive slashes or initial slash if start was 0
...
public:
     ...
     */
    void add_route(const std::string& path_pattern_str, qb::http::method method, std::list<std::shared_ptr<IAsyncTask<SessionType>>> task_chain_list) {
        ...
        for (size_t i = 0; i < segments.size(); ++i) {
            ...
            if (segment_sv[0] == '*') { // Wildcard segment
            ...
            } else if (segment_sv[0] == ':') { // Parameter segment
                if (segment_sv.length() < 2) {
     ...
     */
    [[nodiscard]] std::optional<MatchedRouteInfo<SessionType>> match(const std::string& path_str, qb::http::method method) const {
        ...
        if (!segments.empty()) { // Only needed if there are segments to potentially form a wildcard value
        ...
        find_match_recursive = 
            [&](std::shared_ptr<Node> current_node_ptr, size_t segment_idx, PathParameters current_params) 
            ...
            if (static_child_it != current_node_ptr->static_children.end()) {
                auto res = find_match_recursive(static_child_it->second, segment_idx + 1, current_params); // Pass params by value for fork
                if (res) return res;
            }
            ...
            if (current_node_ptr->param_child) {
                ...
                auto res = find_match_recursive(current_node_ptr->param_child, segment_idx + 1, std::move(params_for_param_branch));
                if (res) return res;
            }
            ...
            if (current_node_ptr->wildcard_child) {
                ...
                for (size_t i = segment_idx; i < segments.size(); ++i) {
                    ...
                    wildcard_captured_value += path_segments_str_for_wildcard[i]; // Use pre-converted strings for safety
                }
                PathParameters params_for_wildcard_branch = current_params;
                params_for_wildcard_branch.set(current_node_ptr->wildcard_child->segment_match, wildcard_captured_value); 
