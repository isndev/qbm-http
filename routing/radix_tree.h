...
private:
    ...
    method_slot(qb::http::method m) noexcept {
    ...
    struct Node {
        ...
        ///< For STATIC nodes: the exact segment string. For PARAMETER/WILDCARD nodes: the name of the parameter/wildcard (e.g., "id",
        ///< "filepath").
    ...
    merge_allowed_methods(std::vector<qb::http::method> &dst, const std::vector<qb::http::method> &src) {
        ...
        std::sort(dst.begin(), dst.end(), [](qb::http::method lhs, qb::http::method rhs) { return method_slot(lhs) < method_slot(rhs); });
        dst.erase(std::unique(dst.begin(), dst.end(),
                              [](qb::http::method lhs, qb::http::method rhs) { return method_slot(lhs) == method_slot(rhs); }),
                  dst.end());
    }
    ...
    split_path_to_segments(std::string_view path_sv) noexcept {
        ...
        // If path_sv was "/foo/", start becomes 1. Loop finds '/' at i=4. segments gets {"foo"}. start becomes 5. start < n (5) is false.
        // Correct.
        return segments;
    }
...
public:
     ...
     */
    RadixTree()
        : _root(std::make_unique<Node>(NodeType::ROOT)) {}
     ...
     */
    void
    add_route(const std::string &path_pattern_str, qb::http::method method,
        ...
        for (size_t i = 0; i < segments.size(); ++i) {
            ...
            if (segment_sv[0] == '*') {
                ...
                if (segment_sv.length() < 2) {
                    throw std::invalid_argument("Wildcard segment must have a name (e.g., *filepath), got: '" + std::string(segment_sv)
                                                + "' in path: " + path_pattern_str);
                }
                if (i != segments.size() - 1) {
                    // Wildcard must be the last segment
                    throw std::invalid_argument("Wildcard segment '" + std::string(segment_sv)
                ...
                if (!seen_capture_names.emplace(std::string(wildcard_name_sv)).second) {
                    throw std::invalid_argument("Duplicate capture name '" + std::string(wildcard_name_sv)
                ...
                // Parameter segment
                if (segment_sv.length() < 2) {
                    throw std::invalid_argument("Parameter segment must have a name (e.g., :id), got: '" + std::string(segment_sv)
                                                + "' in path: " + path_pattern_str);
                }
                _path_segment_storage.emplace_back(segment_sv.substr(1));
                std::string_view p_name_sv = _path_segment_storage.back();
                if (!seen_capture_names.emplace(std::string(p_name_sv)).second) {
                    throw std::invalid_argument("Duplicate capture name '" + std::string(p_name_sv) + "' in path pattern: " + path_pattern_str);
                }
                ...
                if (!current_node->param_child) {
                    ...
                    throw std::invalid_argument("Parameter segment '" + std::string(segment_sv) + "' conflicts with existing parameter ':"
                                                + std::string(current_node->param_name) + "' at the same level in path: " + path_pattern_str);
                }
        ...
        if (slot >= METHOD_SLOT_COUNT) {
            throw std::invalid_argument("Cannot register a handler for an uninitialised / out-of-range HTTP method on path: "
                                        + path_pattern_str);
        }
...
public:
     ...
     */
    [[nodiscard]] std::optional<PathAllowedMethodsInfo<SessionType>>
    allowed_methods(std::string_view path_sv) const {
        PathParameters                params;
        std::vector<std::string_view> segments = split_path_to_segments(path_sv);

        std::function<std::optional<PathAllowedMethodsInfo<SessionType>>(const Node *, size_t, PathParameters)> find_path_recursive =
            [&](const Node *current_node_ptr, size_t segment_idx,
                PathParameters current_params) -> std::optional<PathAllowedMethodsInfo<SessionType>> {
            if (!current_node_ptr) {
                return std::nullopt;
            }

            std::optional<PathAllowedMethodsInfo<SessionType>> result;
            const auto merge_result = [&](std::optional<PathAllowedMethodsInfo<SessionType>> candidate) {
            ...
            if (segment_idx == segments.size()) {
                if (has_any_handler(*current_node_ptr)) {
                    merge_result(PathAllowedMethodsInfo<SessionType>(current_params, collect_allowed_methods(*current_node_ptr)));
                }
                if (current_node_ptr->wildcard_child && has_any_handler(*current_node_ptr->wildcard_child)) {
                    PathParameters final_params_for_wc = current_params;
                    final_params_for_wc.set(current_node_ptr->wildcard_child->segment_match, "");
                    merge_result(PathAllowedMethodsInfo<SessionType>(std::move(final_params_for_wc),
            ...
            auto static_child_it = current_node_ptr->static_children.find(current_path_segment_view);
            if (static_child_it != current_node_ptr->static_children.end()) {
                merge_result(find_path_recursive(static_child_it->second.get(), segment_idx + 1, current_params));
            }

            if (current_node_ptr->param_child) {
                PathParameters params_for_param_branch = current_params;
                params_for_param_branch.set(current_node_ptr->param_name, current_path_segment_view);
                merge_result(find_path_recursive(current_node_ptr->param_child.get(), segment_idx + 1, std::move(params_for_param_branch)));
            }
            ...
            if (current_node_ptr->wildcard_child && has_any_handler(*current_node_ptr->wildcard_child)) {
                ...
                PathParameters params_for_wildcard_branch = current_params;
                params_for_wildcard_branch.set(current_node_ptr->wildcard_child->segment_match, wildcard_captured_view);
                merge_result(PathAllowedMethodsInfo<SessionType>(std::move(params_for_wildcard_branch),
                                                                 collect_allowed_methods(*current_node_ptr->wildcard_child)));
            }
