...
public:
     ...
     */
    Cookie &
    expires(const std::chrono::system_clock::time_point &time_point) {
        _expires = time_point;
        return *this;
    }
     ...
     */
    Cookie &
    expires_in(qb::duration ttl) {
        _expires = std::chrono::system_clock::now() + std::chrono::duration_cast<std::chrono::system_clock::duration>(ttl);
        return *this;
    }
     ...
     */
    Cookie &
    max_age(qb::duration ttl) {
        _max_age = static_cast<int>(std::chrono::duration_cast<std::chrono::seconds>(ttl).count());
        return *this;
    }
     ...
     */
    [[nodiscard]] const std::optional<SameSite> &
    same_site() const noexcept {
        return _same_site;
    }
...
public:
     ...
     */
    [[nodiscard]] const qb::icase_unordered_map<Cookie> &
    all() const {
        return _cookies;
    }
 ...
 */
[[nodiscard]] qb::icase_unordered_map<std::string> parse_cookies(std::string_view header, bool set_cookie_header);
