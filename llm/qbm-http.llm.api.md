...
- `use(MiddlewareHandlerFn, name="...")` / `use(std::shared_ptr<IMiddleware<S>>, name="")` / `template<typename MW,typename...A> use(A&&...)` — global middleware on the root group (`router.h:234`).
...
The verb methods (`get`, `post`, `put`, `del`, `patch`, `options`, `head`) and `use()` each have a **unified overload** that accepts sync OR coroutine handlers via concept-gating (`CoroRouteHandler<F,S>` / `CoroMiddlewareHandler<F,S>`). Pass the lambda directly — no wrapper needed.
