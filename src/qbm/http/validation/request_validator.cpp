...
bool
RequestValidator::validate(qb::http::Request &request, Result &result, const qb::http::PathParameters *path_params) {
                ...
                for (std::string &value_str : query_pair.second) {
                    ...
                    for (const auto &sanitizer_func : it_sanitizers->second) {
                        try {
                            ...
                            query_sanitize_result.add_error("query." + param_name, "sanitizeException.query",
                                                            "Query sanitizer threw exception: " + std::string(e.what()), value_str);
                            overall_valid    = false;
                            sanitizer_failed = true;
                            break;
                        } catch (...) {
                            query_sanitize_result.add_error("query." + param_name, "sanitizeException.query",
                                                            "Query sanitizer threw unknown exception.", value_str);
                            overall_valid    = false;
                            sanitizer_failed = true;
                            break;
                        }
                ...
                for (std::string &value_str : header_pair.second) {
                    ...
                    for (const auto &sanitizer_func : it_sanitizers->second) {
                        try {
                            ...
                            header_sanitize_result.add_error("header." + header_name, "sanitizeException.header",
                                                             "Header sanitizer threw unknown exception.", value_str);
                            overall_valid    = false;
                            sanitizer_failed = true;
                            break;
                        }
    ...
    if (_body_sanitizer && !request.body().empty()) {
        ...
        try {
            ...
            try {
                ...
                sanitize_body_result.add_error("body", "sanitizeException.body", "Body sanitizer threw exception: " + std::string(e.what()),
                                               request.body().as<std::string_view>());
                overall_valid            = false;
                body_sanitization_failed = true;
            } catch (...) {
                sanitize_body_result.add_error("body", "sanitizeException.body", "Body sanitizer threw unknown exception.",
                                               request.body().as<std::string_view>());
                overall_valid            = false;
                body_sanitization_failed = true;
            }
            ...
            sanitize_body_result.add_error("body", "invalidFormat.sanitize",
                                           "Request body is not valid JSON, cannot apply body sanitizers. Error: " + std::string(e.what()),
                                           request.body().as<std::string_view>());
            overall_valid = false;
        }
    ...
    if (_body_schema_validator) {
        ...
        if (request.body().empty()) {
            ...
            if (!_body_schema_validator->validate(null_json_value, body_val_result)) {
                ...
                for (const auto &err : body_val_result.errors()) {
                    if (err.rule_violated == "type"
                        && (err.message.find("null") != std::string::npos || err.message.find("object") != std::string::npos
                            || err.message.find("array") != std::string::npos)) {
                ...
                if (body_val_result.errors().empty() || !type_error_related_to_null) {
                    ...
                    body_val_result.add_error("body", "contentRequired",
                                              "Request body is empty, but the schema expects a non-null structure (e.g., object or array).",
                                              nullptr);
                }
            }
        } else {
            try {
                ...
                body_val_result.add_error("body", "invalidFormat.validate", "Request body is not valid JSON. Error: " + std::string(e.what()),
                                          request.body().as<std::string_view>());
                overall_valid = false;
            }
        }
    ...
    if (_query_param_validator) {
        ...
        for (const auto &[param_name_defined, rules] : _query_param_validator->get_param_definitions()) {
            ...
            if (it_query != request.queries().end() && !it_query->second.empty()) {
                ...
                for (const std::string &value_str : it_query->second) {
                    ...
                    _query_param_validator->validate_single(param_name_defined, std::make_optional(value_str), rules, single_value_result,
                                                            "query");
        ...
        if (_query_param_validator->is_strict_mode()) {
            for (const auto &[name_in_request, value_list_in_request] : request.queries()) {
                if (!value_list_in_request.empty()
                    && _query_param_validator->get_param_definitions().find(name_in_request)
                           == _query_param_validator->get_param_definitions().end()) {
                    query_param_val_result.add_error("query." + name_in_request, "unexpectedParameter", "Unexpected query parameter provided.",
                                                     value_list_in_request.front());
                    overall_valid = false;
                }
            }
        }
    ...
    if (_header_validator) {
        ...
        for (const auto &[header_name_defined, rules] : _header_validator->get_param_definitions()) {
            ...
            if (it_header != request.headers().end() && !it_header->second.empty()) {
                for (const std::string &value_str : it_header->second) {
                    Result single_value_result = result.make_child();
                    _header_validator->validate_single(header_name_defined, std::make_optional(value_str), rules, single_value_result,
                                                       "header");
                    if (!single_value_result.success()) {
        ...
        if (_header_validator->is_strict_mode()) {
            for (const auto &[name_in_request, value_list_in_request] : request.headers()) {
                if (!value_list_in_request.empty()
                    && _header_validator->get_param_definitions().find(name_in_request) == _header_validator->get_param_definitions().end()) {
                    header_val_result.add_error("header." + name_in_request, "unexpectedParameter", "Unexpected header provided.",
                                                value_list_in_request.front());
                    overall_valid = false;
                }
            }
    ...
    if (_path_param_validator && path_params) {
        ...
        for (const auto &[param_name_defined, rules] : _path_param_validator->get_param_definitions()) {
            ...
            path_param_val_result.add_error("path." + rules.name, "required", "Path parameter context is required for validation.", nullptr);
        }
        result.merge(path_param_val_result);
        overall_valid = false;
    }
