...
class TransformMiddleware final : public IMiddleware<SessionType> {
public:
     ...
     */
    explicit TransformMiddleware(RequestTransformer request_transformer = nullptr, std::string instance_name = "TransformMiddleware") noexcept(
        noexcept(std::function<void(Request &request)>(nullptr)) && noexcept(std::string(std::string())))
    ...
    process(ContextPtr ctx) override {
        // User transformer can throw
        if (_request_transformer) {
            try {
                _request_transformer(ctx->request());
            } catch (...) {
                // Catch any exception from the user-provided transformer
...
transform_middleware(typename TransformMiddleware<SessionType>::RequestTransformer request_transformer = nullptr,
                     const std::string                                            &name                = "TransformMiddleware") {
    return std::make_shared<TransformMiddleware<SessionType>>(std::move(request_transformer), name);
}
} // namespace qb::http
