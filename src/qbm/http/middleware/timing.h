...
class TimingMiddleware final : public IMiddleware<SessionType> {
public:
    ...
    process(ContextPtr ctx) override {
        ...
        ctx->add_lifecycle_hook([callback = std::move(callback), middleware_name = std::move(middleware_name),
            ...
            if (point == HookPoint::PRE_RESPONSE_SEND) {
                if (auto start_time_opt = ctx_ref.template get<TimePoint>(key)) {
                    ...
                    auto duration_for_header = std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(end_time - start_time);
                    ctx_ref.response().set_header("X-Response-Time", std::to_string(duration_for_header.count()) + "ms");
...
timing_middleware(typename TimingMiddleware<SessionType>::TimingCallback callback, const std::string &name = "TimingMiddleware") {
    return std::make_shared<TimingMiddleware<SessionType>>(std::move(callback), name);
}
} // namespace qb::http
