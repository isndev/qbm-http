...
constexpr std::array<const char *, 12> MONTH_NAMES_LONG = {"January", "February", "March",     "April",   "May",      "June",
                                                           "July",    "August",   "September", "October", "November", "December"};

constexpr std::array<const char *, 7> DAY_NAMES = {"Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"};
 ...
 */
int
month_to_num(std::string_view month) {
 ...
 */
std::string
format_http_date(std::chrono::system_clock::time_point const tp) noexcept {
    ...
    oss << DAY_NAMES[tm_value.tm_wday] << ", " << (tm_value.tm_mday < 10 ? "0" : "") << tm_value.tm_mday << " " << MONTH_NAMES[tm_value.tm_mon]
        << " " << (tm_value.tm_year + 1900) << " " << (tm_value.tm_hour < 10 ? "0" : "") << tm_value.tm_hour << ":"
        << (tm_value.tm_min < 10 ? "0" : "") << tm_value.tm_min << ":" << (tm_value.tm_sec < 10 ? "0" : "") << tm_value.tm_sec << " "
        << "GMT";
 ...
 */
std::string
format_cookie_date(std::chrono::system_clock::time_point const tp) noexcept {
 ...
 */
std::optional<std::chrono::system_clock::time_point>
parse_rfc1123_date(std::string_view str) {
    ...
    auto pos = str.find(',');
    if (pos != 3 || pos + 1 >= str.size() || str[pos + 1] != ' ')
        return std::nullopt;
    ...
    if (pos + 2 > str.size())
        return std::nullopt;
    int day = 0;
    if (std::from_chars(str.data() + pos, str.data() + pos + 2, day).ec != std::errc{})
        return std::nullopt;
    if (pos + 2 >= str.size() || str[pos + 2] != ' ')
        return std::nullopt;
    ...
    if (pos + 4 > str.size())
        return std::nullopt;
    int year = 0;
    if (std::from_chars(str.data() + pos, str.data() + pos + 4, year).ec != std::errc{})
        return std::nullopt;
    if (pos + 4 >= str.size() || str[pos + 4] != ' ')
        return std::nullopt;
    ...
    if (std::from_chars(str.data() + pos, str.data() + pos + 2, hour).ec != std::errc{})
        return std::nullopt;
    if (str[pos + 2] != ':')
        return std::nullopt;
    if (std::from_chars(str.data() + pos + 3, str.data() + pos + 5, minute).ec != std::errc{})
        return std::nullopt;
    if (str[pos + 5] != ':')
        return std::nullopt;
    if (std::from_chars(str.data() + pos + 6, str.data() + pos + 8, second).ec != std::errc{})
        return std::nullopt;

    pos += 8;
    if (pos + 4 != str.size() || str.substr(pos, 4) != " GMT")
        return std::nullopt;
 ...
 */
std::optional<std::chrono::system_clock::time_point>
parse_rfc850_date(std::string_view str) {
    // Format: "Sunday, 06-Nov-94 08:49:37 GMT"
    auto pos = str.find(',');
    if (pos == std::string_view::npos || pos + 1 >= str.size() || str[pos + 1] != ' ')
        return std::nullopt;
    ...
    if (pos + 2 > str.size())
        return std::nullopt;
    int year = 0;
    if (std::from_chars(str.data() + pos, str.data() + pos + 2, year).ec != std::errc{})
        return std::nullopt;
    if (year < 70) {
 ...
 */
std::optional<std::chrono::system_clock::time_point>
parse_asctime_date(std::string_view str) {
    // Format: "Sun Nov  6 08:49:37 1994"
    if (str.size() != 24 || str[3] != ' ' || str[7] != ' ')
        return std::nullopt;
    ...
    size_t day_len = 0;
    while (pos + day_len < str.size() && std::isdigit(static_cast<unsigned char>(str[pos + day_len]))) {
        day_len++;
    }
    ...
    int day = 0;
    if (std::from_chars(str.data() + pos, str.data() + pos + day_len, day).ec != std::errc{})
        return std::nullopt;
    if (pos + day_len >= str.size() || str[pos + day_len] != ' ')
        return std::nullopt;
