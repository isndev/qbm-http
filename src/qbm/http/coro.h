...
class http_awaiter {
    static_assert(std::is_default_constructible_v<T>, "http_awaiter<T>: T must be default-constructible for cancellation safety");

    T                                  _result{};
    std::coroutine_handle<>            _handle{};
    std::shared_ptr<bool>              _alive{std::make_shared<bool>(true)};
    std::shared_ptr<std::atomic<bool>> _completed{std::make_shared<std::atomic<bool>>(false)};
    Operation                          _op;
...
public:
    ...
    [[nodiscard]] T
    await_resume() noexcept(std::is_nothrow_move_constructible_v<T>) {
