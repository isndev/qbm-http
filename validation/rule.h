...
namespace qb::http::validation {
    ...
    public:
        ...
        // Changed ValidationResult to Result
        /**
    ...
    using ItemsRuleLogic = std::variant<std::shared_ptr<SchemaValidator>, std::vector<std::shared_ptr<
        SchemaValidator> > >;
    ...
    public:
        explicit ItemsRule(ItemsRuleLogic logic,
                           std::variant<bool, std::shared_ptr<SchemaValidator> > additional_items_policy = true);
    ...
    public:
        using CustomValidateFn = std::function<bool(const qb::json &value, const std::string &field_path,
                                                    Result &result)>;
