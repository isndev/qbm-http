...
namespace {
[[nodiscard]] bool
ascii_is_space(unsigned char c) noexcept {
    return c == ' ' || c == '\t' || c == '\n' || c == '\r' || c == '\f' || c == '\v';
}

[[nodiscard]] bool
ascii_is_alnum(unsigned char c) noexcept {
...
void
Sanitizer::add_rule(const std::string &field_path, SanitizerFunction func) {
    if (func) {
...
split_path_for_sanitizer(const std::string &path) {
...
void
Sanitizer::apply_sanitizers_to_node(qb::json &node, const std::vector<SanitizerFunction> &funcs) const {
    if (node.is_string()) {
...
void
Sanitizer::traverse_and_apply(qb::json &current_node, const std::vector<std::string> &path_segments, size_t segment_idx,
...
void
Sanitizer::sanitize(qb::json &data) const {
    for (const auto &[path_key, funcs_to_apply] : _rules) {
        if (funcs_to_apply.empty())
            continue;
...
trim() noexcept {
    return [](const std::string &input) -> std::string {
        ...
        auto first_char = std::find_if_not(input.begin(), input.end(), [](unsigned char c) { return ascii_is_space(c); });
        // If the string is all whitespace, return an empty string
        if (first_char == input.end()) {
            return "";
        }
        // Find the last non-whitespace character
        auto last_char = std::find_if_not(input.rbegin(), input.rend(), [](unsigned char c) { return ascii_is_space(c); }).base();
        return std::string(first_char, last_char);
    };
}

SanitizerFunction
to_lower_case() noexcept {
    return [](const std::string &input) -> std::string {
        std::string output = input;
        std::transform(output.begin(), output.end(), output.begin(), [](char c) { return qb::http::utility::ascii_to_lower(c); });
        return output;
    };
}

SanitizerFunction
to_upper_case() noexcept {
    return [](const std::string &input) -> std::string {
        std::string output = input;
        std::transform(output.begin(), output.end(), output.begin(), [](char c) { return ascii_to_upper(c); });
        return output;
    };
}

SanitizerFunction
escape_html() noexcept {
    return [](const std::string &input) -> std::string {
        std::string buffer;
