...
to_result_policy(const SchemaValidator::ErrorValuePolicy policy) {
    switch (policy) {
        case SchemaValidator::ErrorValuePolicy::None:
            return Result::ErrorValuePolicy::None;
        case SchemaValidator::ErrorValuePolicy::Preview:
            return Result::ErrorValuePolicy::Preview;
        case SchemaValidator::ErrorValuePolicy::Full:
        default:
            return Result::ErrorValuePolicy::Full;
    }
}
} // namespace
...
bool
SchemaValidator::validate(const qb::json &data_to_validate, Result &result) const {
...
SchemaValidator &
SchemaValidator::set_error_value_policy(ErrorValuePolicy policy, std::size_t preview_bytes) noexcept {
...
bool
SchemaValidator::apply_primitive_rules(const qb::json &value, const qb::json &schema_node, const std::string &path, Result &result) const {
...
SchemaValidator::validate_recursive(const qb::json &current_value, const qb::json &current_schema, const std::string &current_path,
    ...
    if (!current_schema.is_object()) {
        result.add_error(current_path.empty() ? "_schema" : current_path + "._schema", "invalidSchemaType",
                         "Schema definition at this level must be an object.", std::make_optional(current_schema));
        return false;
    }
    ...
    if (type_ok) {
        if (current_value.is_object()) {
            ...
            structural_ok =
                properties_check_passed && required_check_passed && additional_properties_check_passed && property_names_check_passed;
    ...
    if (current_schema.contains("allOf")) {
        if (!validate_allOf_keyword(current_value, current_schema["allOf"], current_path, result))
            logical_combinators_ok = false;
    }
    if (logical_combinators_ok && current_schema.contains("anyOf")) {
        if (!validate_anyOf_keyword(current_value, current_schema["anyOf"], current_path, result))
            logical_combinators_ok = false;
    }
    if (logical_combinators_ok && current_schema.contains("oneOf")) {
        if (!validate_oneOf_keyword(current_value, current_schema["oneOf"], current_path, result))
            logical_combinators_ok = false;
    }
    if (logical_combinators_ok && current_schema.contains("not")) {
        if (!validate_not_keyword(current_value, current_schema["not"], current_path, result))
            logical_combinators_ok = false;
    }
...
bool
SchemaValidator::validate_type_keyword(const qb::json &value, const qb::json &schema_type_def, const std::string &path, Result &result) const {
    if (schema_type_def.is_string()) {
        ...
        for (const auto &type_option_json : schema_type_def) {
            if (!type_option_json.is_string()) {
                result.add_error(path, "schemaError.type", "Elements in schema 'type' array must be strings.",
            ...
            if (type_str == "string")
                ...
                result.add_error(path, "schemaError.type", "Unknown type specified in schema type array: " + type_str,
                                 std::make_optional(type_option_json));
                return false;
            }
        ...
        result.add_error(path, "type", "Value does not match any of the allowed types: " + schema_type_def.dump(), std::make_optional(value));
        return false;
    }
    result.add_error(path, "type", "Schema 'type' keyword must be a string or an array of strings.", std::make_optional(schema_type_def));
    return false;
}

bool
SchemaValidator::validate_properties_keyword(const qb::json &value, const qb::json &properties_def, const std::string &path,
                                             Result &result) const {
    if (!value.is_object())
        return true;
...
bool
SchemaValidator::validate_required_keyword(const qb::json &value, const qb::json &required_def, const std::string &path, Result &result) const {
    if (!value.is_object())
        return true;
    if (!required_def.is_array()) {
        result.add_error(path, "schemaError.required", "'required' keyword must be an array of strings.", std::make_optional(required_def));
        return false;
    }
...
bool
SchemaValidator::validate_items_keyword(const qb::json &value, const qb::json &schema_node, const std::string &path, Result &result) const {
    if (!value.is_array())
        return true;
    if (!schema_node.contains("items"))
        return true;

    const qb::json &items_def = schema_node["items"];
    bool            is_valid  = true;

    if (items_def.is_object()) {
        ...
        for (size_t i = 0; i < value.size(); ++i) {
            ...
            if (i < items_def.size()) {
                ...
                // Past the defined tuple schemas, check additionalItems
...
bool
SchemaValidator::validate_additional_properties_keyword(const qb::json &value, const qb::json &schema_node, const std::string &path,
                                                        Result &result) const {
    if (!value.is_object())
        return true;

    // Ensure additionalProperties is present in the schema node itself, not its parent
    if (!schema_node.contains("additionalProperties"))
        return true; // No restriction if not present
    ...
    for (auto const &[key, val] : value.items()) {
        ...
        if (additional_props_def.is_boolean()) {
            if (!additional_props_def.get<bool>()) {
                result.add_error(prop_path, "additionalProperties", "Additional property '" + key + "' not allowed.", std::make_optional(val));
                is_valid = false;
            }
            ...
            if (!additional_validator.validate(val, sub_result)) {
                for (const auto &err : sub_result.errors()) {
                    ...
                    if (!err.field_path.empty()) {
                        ...
                        } else if (prop_path.length() > 0 && err.field_path.length() > 0 && prop_path.back() != '.'
                                   && err.field_path.front() != '.') {
                            new_path += ".";
            ...
            result.add_error(path, "schemaError.additionalProperties", "'additionalProperties' must be a boolean or a schema object.",
                             std::make_optional(additional_props_def));
            return false;
        }
    }
    return is_valid;
...
bool
SchemaValidator::validate_allOf_keyword(const qb::json &value, const qb::json &allOf_def, const std::string &path, Result &result) const {
...
bool
SchemaValidator::validate_anyOf_keyword(const qb::json &value, const qb::json &anyOf_def, const std::string &path, Result &result) const {
...
bool
SchemaValidator::validate_oneOf_keyword(const qb::json &value, const qb::json &oneOf_def, const std::string &path, Result &result) const {
    ...
    for (const auto &sub_schema : oneOf_def) {
        if (!sub_schema.is_object()) {
            result.add_error(path, "schemaError.oneOf.item", "Items in 'oneOf' array must be schema objects.", std::make_optional(sub_schema));
            continue;
        }
    ...
    if (S_VAL_P_151_S_1_E_151_S_1 == 0) {
        result.add_error(path, "oneOf", "Value does not validate against exactly one of the specified schemas (matched 0).",
                         std::make_optional(value));
    } else {
        result.add_error(path, "oneOf",
                         "Value validates against more than one of the specified schemas (matched " + std::to_string(S_VAL_P_151_S_1_E_151_S_1)
                             + ").",
                         std::make_optional(value));
    }
    return false;
...
bool
SchemaValidator::validate_not_keyword(const qb::json &value, const qb::json &not_def, const std::string &path, Result &result) const {
