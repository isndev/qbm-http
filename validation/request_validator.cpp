        ...
        if (_body_sanitizer && !request.body().empty()) {
            ...
            try {
                ...
                sanitize_body_result.add_error("body", "invalidFormat.sanitize",
                                               ...
                                               std::string(e.what()), request.body().as<std::string_view>());
                overall_valid = false;
            }
        ...
        if (_body_schema_validator) {
            ...
            if (request.body().empty()) {
                ...
                if (!_body_schema_validator->validate(null_json_value, body_val_result)) {
                    ...
                    for (const auto &err: body_val_result.errors()) {
                        if (err.rule_violated == "type" &&
                            (err.message.find("null") != std::string::npos || err.message.find("object") !=
                             std::string::npos || err.message.find("array") != std::string::npos)) {
                ...
                try {
                    ...
                    body_val_result.add_error("body", "invalidFormat.validate",
                                              "Request body is not valid JSON. Error: " + std::string(e.what()),
        ...
        if (_query_param_validator) {
            ...
            if (_query_param_validator->is_strict_mode()) {
                for (const auto &[name_in_request, value_list_in_request]: request.queries()) {
                    if (!value_list_in_request.empty() && _query_param_validator->get_param_definitions().
                        find(name_in_request) == _query_param_validator->get_param_definitions().end()) {
                        query_param_val_result.add_error("query." + name_in_request, "unexpectedParameter",
                                                         "Unexpected query parameter provided.",
        ...
        if (_header_validator) {
            ...
            if (_header_validator->is_strict_mode()) {
                for (const auto &[name_in_request, value_list_in_request]: request.headers()) {
                    if (!value_list_in_request.empty() && _header_validator->get_param_definitions().
                        find(name_in_request) == _header_validator->get_param_definitions().end()) {
                        header_val_result.add_error("header." + name_in_request, "unexpectedParameter",
                                                    "Unexpected header provided.", value_list_in_request.front());
                        overall_valid = false;
                    }
                }
            }
