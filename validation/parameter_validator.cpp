...
namespace qb::http::validation {
    ...
    qb::json ParameterValidator::parse_value(const std::string &input_value, DataType target_type,
                                             const std::string &field_path, Result &result, bool &success) const {
        success = true;
        switch (target_type) {
            ...
            case DataType::ANY:
            default:
                result.add_error(field_path, "type",
                                 "Unsupported target type for parameter parsing: " + TypeRule::data_type_to_string(
                                     target_type), input_value);
                success = false;
                return nullptr;
        }
    }
    ...
    qb::json ParameterValidator::validate_single(
        ...
        if (rules.custom_parser) {
            ...
            if (!parse_success) {
                ...
                if (result.success()) {
                    result.add_error(field_path, "customParse", "Failed to parse using custom parser.",
                                     current_value_str);
                }
                return nullptr;
            }
    ...
    bool ParameterValidator::validate(const qb::icase_unordered_map<std::string> &params, Result &result,
        ...
        if (_strict_mode) {
            for (const auto &[name_in_request, value_in_request]: params) {
                if (_param_definitions.find(name_in_request) == _param_definitions.end()) {
                    result.add_error(param_source_name + "." + name_in_request, "unexpectedParameter",
                                     "Unexpected parameter provided.", value_in_request);
                    overall_success = false;
                }
            }
        }
