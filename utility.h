...
split_string(std::string_view str, std::string_view delimiters,
    ...
    if (reserve > 0) { // Ensure reserve is only called if reserve > 0
    ...
    while (first != str.end()) {
        ...
        if (first != second) { // Ensure non-empty token
...
split_string_by(std::string_view str, std::string_view boundary,
    ...
    while (current_pos <= str.length()) { // Use <= to handle trailing empty part if str ends with boundary
...
join(const std::vector<T>& strings, std::string_view delimiter) {
    ...
    if (!strings.empty()) {
        ...
        if (total_len > 0) { // Avoid reserving 0
