...
private:
    ...
    std::chrono::seconds _clock_skew_tolerance = std::chrono::seconds(0); // Default: no tolerance
...
public:
     ...
     */
    AuthOptions& secret_key(const std::string& secret) {
     ...
     * @return Référence à l'objet AuthOptions pour chaînage
     */
    AuthOptions& secret_key(const std::vector<unsigned char>& secret) {
        _secret_key = secret;
        return *this;
    }
     ...
     * @return Référence à l'objet AuthOptions pour chaînage
     */
    AuthOptions& public_key(const std::string& key) {
        _public_key = key;
        return *this;
    }
     ...
     * @return Référence à l'objet AuthOptions pour chaînage
     */
    AuthOptions& private_key(const std::string& key) {
        _private_key = key;
        return *this;
    }
     ...
     * @return Référence à l'objet AuthOptions pour chaînage
     */
    AuthOptions& algorithm(Algorithm alg) {
        _algorithm = alg;
        return *this;
    }
     ...
     * @return Référence à l'objet AuthOptions pour chaînage
     */
    AuthOptions& token_expiration(std::chrono::seconds seconds) {
     ...
     * @return Référence à l'objet AuthOptions pour chaînage
     */
    AuthOptions& token_issuer(const std::string& issuer) {
     ...
     * @return Référence à l'objet AuthOptions pour chaînage
     */
    AuthOptions& token_audience(const std::string& audience) {
     ...
     * @return Référence à l'objet AuthOptions pour chaînage
     */
    AuthOptions& auth_header_name(const std::string& name) {
     ...
     * @return Référence à l'objet AuthOptions pour chaînage
     */
    AuthOptions& auth_scheme(const std::string& scheme) {
        _auth_scheme = scheme;
        return *this;
    }
     ...
     * @return Référence à l'objet AuthOptions pour chaînage
     */
    AuthOptions& require_signature_verification(bool verify) {
     ...
     * @return Référence à l'objet AuthOptions pour chaînage
     */
    AuthOptions& verify_expiration(bool verify) {
     ...
     * @return Référence à l'objet AuthOptions pour chaînage
     */
    AuthOptions& verify_not_before(bool verify) {
     ...
     * @return Référence à l'objet AuthOptions pour chaînage
     */
    AuthOptions& clock_skew_tolerance(std::chrono::seconds tolerance) {
        _clock_skew_tolerance = tolerance;
        return *this;
    }
    
    // Getters
    const std::vector<unsigned char>& get_secret_key() const { return _secret_key; }
    ...
    std::chrono::seconds get_token_expiration() const { return _token_expiration; }
    std::chrono::seconds get_clock_skew_tolerance() const { return _clock_skew_tolerance; }
    const std::string& get_token_issuer() const { return _token_issuer; }
    const std::string& get_token_audience() const { return _token_audience; }
    const std::string& get_auth_header_name() const { return _auth_header_name; }
    const std::string& get_auth_scheme() const { return _auth_scheme; }
    bool get_require_signature_verification() const { return _require_signature_verification; }
    bool get_verify_expiration() const { return _verify_expiration; }
    bool get_verify_not_before() const { return _verify_not_before; }
    bool get_verify_issuer() const { return _verify_issuer; }
    bool get_verify_audience() const { return _verify_audience; }
};
...
public:
     ...
     * @return Les informations utilisateur extraites, ou nullopt si le token est invalide
     */
     ...
     */
    explicit AuthManager(const AuthOptions& options = AuthOptions()) : _options(options) {}
    ...
    typename Router::middleware_handler authenticate() const {
        return [this](typename Router::Context& ctx) {
            const auto& auth_header = ctx.request.header(_options.get_auth_header_name());
     ...
     */
    template <typename Router>
    typename Router::middleware_handler authorize(const std::vector<std::string>& roles, bool require_all = false) const {
        return [this, roles, require_all](typename Router::Context& ctx) {
            ...
            bool authorized = require_all ? user.has_all_roles(roles) : user.has_any_role(roles);
     ...
     */
    const AuthOptions& get_options() const { return _options; }
    
    /**
     * @brief Mettre à jour les options d'authentification
     * @param options Les nouvelles options
     */
    void set_options(const AuthOptions& options) { _options = options; }
};
