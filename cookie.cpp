...
validate_cookie_for_serialization(const Cookie &cookie) {
    if (cookie.name().empty() || !std::all_of(cookie.name().begin(), cookie.name().end(), [](char c) {
...
is_cookie_attribute(std::string_view name, bool set_cookie_header) noexcept {
    ...
    if (set_cookie_header) {
        ...
        static const char *known_attributes[] = {"Comment", "Domain",  "Max-Age",  "Path",    "Secure",
...
parse_cookies(const char *ptr, const size_t len, bool set_cookie_header) {
    ...
    if (parse_state == CookieParseState::COOKIE_PARSE_VALUE && (value_quote_character != '\0' || value_escape)) {
 ...
 */
std::optional<Cookie>
parse_set_cookie(std::string_view header) {
    if (header.empty()) {
        return std::nullopt;
    }
    ...
    while (!remaining_attrs.empty()) {
        ...
        std::string_view current_attr_pair = utility::trim_http_whitespace(remaining_attrs.substr(0, next_attr_semi));
