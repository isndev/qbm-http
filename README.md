...
- **`listen()` returns "is listening", not "no error" — check it.** The bind reports a syscall-style `0` for success, so the value handed back is the negation, and it is `[[nodiscard]]` for that reason. On success the accept watcher is armed too: `listen()` binds **and** starts, the same contract as the qb-io acceptor it derives from, with `listen_no_start()` for a server that must finish wiring first. Until 3.0 it bound *without* starting, so a missing `start()` left the socket bound, the actor active and nothing ever accepted; a `start()` you already have is harmless.
