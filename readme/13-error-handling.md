...
- **Protocol-level framing errors** happen *before* routing. The llhttp-based parser (or the WebSocket/HTTP/2 framer) rejects a malformed or oversized message by calling `not_ok()` on the protocol object, which tears down the connection. There is no `Context`, no route, and no error chain — the connection simply closes. HTTP/3 refuses the **stream** for what the module itself checks (a field count or length over the limits, an unknown method, a forbidden trailer: a reset with `H3_MESSAGE_ERROR`, RFC 9114 §4.1.2, the other requests on the connection untouched) and closes the connection only for what nghttp3's own validator rejects, the library's contract — see [the HTTP/3 page](19-http3-protocol.md).
