...
> **`PatternRule` is a ReDoS surface.** It uses `std::regex_match` (full match, not search) and has no execution timeout. To bound cost it rejects inputs longer than 256 KiB (`MAX_REGEX_INPUT_LENGTH`) and caps pattern length at 1024 characters, but a pathological pattern can still be super-linear. Prefer linear-time patterns.
