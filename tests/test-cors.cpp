...
TEST_F(CorsTest, CustomOrigins) {
    ...
    router->enable_cors(qb::http::CorsOptions().origins(
...
TEST_F(CorsTest, WildcardVsExplicitOrigin) {
    ...
    router->enable_cors(qb::http::CorsOptions().credentials(
        qb::http::CorsOptions::AllowCredentials::Yes));
    ...
    // With credentials, even with wildcard origin, we should get the specific origin
    // back
    EXPECT_EQ(session->header("Access-Control-Allow-Origin"), "https://example.com");
    EXPECT_EQ(session->header("Access-Control-Allow-Credentials"), "true");
}
...
TEST_F(CorsTest, DifferentRequestMethods) {
    ...
    router->DELETE("/test",
                   [](auto &ctx) { ctx.response.status_code = HTTP_STATUS_NO_CONTENT; });
