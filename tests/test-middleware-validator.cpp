...
protected:
    ...
    qb::http::Request create_request(const std::string &path, qb::http::method method = qb::http::method::POST,
...
TEST_F(ValidationMiddlewareTest, MultipleValidationFailures) {
    ...
    _request_validator->for_header("X-Client-Version",
                                   ParameterRuleSet("X-Client-Version").add_rule(std::make_shared<MinLengthRule>(3)));
...
TEST_F(ValidationMiddlewareTest, InvalidPathParamFails) {
    _request_validator->for_path_param(
        "orderId",
...
TEST_F(ValidationMiddlewareTest, QueryParamSanitizationByMiddleware) {
    ...
    _request_validator->for_query_param(
        "name", ParameterRuleSet("name").set_type(DataType::STRING).add_rule(std::make_shared<MinLengthRule>(3)));
