...
TEST_F(RouterAdvancedTest, Middleware) {
    ...
    // Test with no auth header
    {
        ...
        EXPECT_EQ(session->_response.body().as<std::string>(), "Unauthorized: No token provided");
    }
    ...
    // Test with auth header
    {
        ...
        EXPECT_EQ(session->_response.body().as<std::string>(), "Protected content for user: user123");
    }
}
...
TEST_F(RouterAdvancedTest, RouteGroups) {
    ...
    
    {
        ...
        EXPECT_TRUE(router->route(*session, req));
        EXPECT_EQ(session->_response.status_code, HTTP_STATUS_OK);
        EXPECT_EQ(session->_response.body().as<std::string>(), "API v1 - User details: 123");
    }
    ...
    // Test v2 endpoint
    {
        ...
        EXPECT_TRUE(router->route(*session, req));
        EXPECT_EQ(session->_response.status_code, HTTP_STATUS_OK);
        EXPECT_EQ(session->_response.body().as<std::string>(), "API v2 - Users list (improved)");
    }
}

// Test route priorities
TEST_F(RouterAdvancedTest, RoutePriorities) {
    // Note: Since we can't directly set priorities, we rely on the order of route registration
    // More specific routes should be registered after catch-all routes to ensure they have priority
...
TEST_F(RouterAdvancedTest, Cache) {
    ...
    // Deuxième requête - le handler devrait s'exécuter à nouveau (pas de cache)
    {
        ...
        EXPECT_TRUE(router->route(*session, req));
        EXPECT_EQ(counter, 2);  // Le compteur devrait augmenter car la route est appelée à nouveau
...
TEST_F(RouterAdvancedTest, ErrorHandlers) {
    // Add a 404 handler
    router->onError(HTTP_STATUS_NOT_FOUND, [](Context& ctx) {
    ...
    // Test 404 handler
    {
        ...
        EXPECT_EQ(session->_response.body().as<std::string>(), "Custom 404: Page not found");
    }
}
...
TEST_F(RouterAdvancedTest, RateLimiting) {
    ...
    struct RateLimiter {
        ...
        RateLimiter(int max) : max_requests(max) {}
    ...
    router->use([limiter](Context& ctx) {
        ...
        std::string ip = ctx.get<std::string>("ip", "unknown");
