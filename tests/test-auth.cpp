...
std::string base64_encode_str(const std::string& input) {
...
TEST(AuthManagerTest, DifferentAlgorithms) {
    ...
    std::string ed25519_private_key = readFileContents(keys_path + "ed25519_private.pem");
...
TEST(AuthManagerTest, MalformedTokens) {
    ...
    std::vector<std::string> malformed_tokens = {
        ...
        "too.many.parts.here",
        "bad-base64.value!",
        "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.bad-payload", // Valid header, invalid payload
...
TEST(AuthManagerTest, AlgorithmMismatch) {
    ...
    EXPECT_FALSE(result.has_value()) << "Token signed with RSA should not be verifiable with HMAC";
}
...
TEST(AuthManagerTest, EdgeCases) {
    ...
    // User with empty roles array
    {
        ...
        EXPECT_TRUE(verify_result.value().has_all_roles({})); // Empty requirement is always true
    }
    ...
    // User with special characters in metadata
    {
        ...
        ASSERT_TRUE(verify_result.has_value());
        EXPECT_EQ(verify_result.value().metadata.at("special_chars"), test_user.metadata.at("special_chars"));
    }
}
...
TEST(AuthManagerTest, ComplexJsonMetadata) {
    ...
    test_user.metadata["json_object"] = "{\"name\":\"John\",\"age\":30,\"city\":\"New York\"}";
    test_user.metadata["nested_json"] = "{\"employees\":[{\"name\":\"Alice\",\"dept\":\"IT\"},{\"name\":\"Bob\",\"dept\":\"HR\"}]}";
...
TEST(AuthManagerTest, TokenPerformance) {
    ...
    std::cout << "Generated " << NUM_TOKENS << " tokens in " << gen_duration << "ms (avg: " 
              << static_cast<double>(gen_duration) / NUM_TOKENS << "ms per token)" << std::endl;
    ...
    std::cout << "Verified " << NUM_TOKENS << " tokens in " << verify_duration << "ms (avg: " 
              << static_cast<double>(verify_duration) / NUM_TOKENS << "ms per token)" << std::endl;
...
TEST(AuthManagerTest, TokenRefresh) {
    ...
    EXPECT_TRUE(refreshed_user_verified.metadata.find("refreshed_at") != refreshed_user_verified.metadata.end());
...
TEST(AuthManagerTest, AuthorizationFlow) {
    ...
    access_options.token_expiration(std::chrono::minutes(15)); // Short-lived access token
    ...
    EXPECT_FALSE(new_validation.value().metadata.find("token_type") != new_validation.value().metadata.end());
...
TEST(AuthManagerTest, PasswordValidation) {
    ...
    auto is_valid_password = [](const std::string& password) -> bool {
        ...
        bool has_uppercase = std::any_of(password.begin(), password.end(), [](char c) { return std::isupper(c); });
        bool has_lowercase = std::any_of(password.begin(), password.end(), [](char c) { return std::islower(c); });
    ...
    user.id = "user-12345";
    user.username = "test_user";
    user.metadata["password_hash"] = "hashed_password_value"; // In a real system, this would be hashed
    
    auto token = auth_manager.generate_token(user);
    auto verify_result = auth_manager.verify_token(token);
    
    ASSERT_TRUE(verify_result.has_value());
    EXPECT_EQ(verify_result.value().metadata.at("password_hash"), "hashed_password_value");
    
    // Test simulated password verification
    auto verify_password = [](const std::string& password, const std::string& hash) -> bool {
...
TEST(AuthManagerTest, TokenRefreshStrategies) {
    ...
    std::string token_family = refresh_result.value().metadata.at("refresh_token_family");
    std::string token_version = refresh_result.value().metadata.at("refresh_token_version");
    
    // Check if token has been used before
    bool token_used = used_refresh_tokens[token_family].find(token_version) != used_refresh_tokens[token_family].end();
    ...
    bool reuse_detected = used_refresh_tokens[token_family].find(token_version) != used_refresh_tokens[token_family].end();
    EXPECT_TRUE(reuse_detected);
    
    // Test sliding window expiration for refresh tokens
    AuthOptions sliding_options = refresh_options;
    sliding_options.token_expiration(std::chrono::hours(1)); // Short expiration for testing
...
TEST(AuthManagerTest, RoleBasedAccessControl) {
    ...
    auto has_permission = [&role_hierarchy](const std::vector<std::string>& user_roles, const std::string& required_role) -> bool {
        // Direct role check
        if (std::find(user_roles.begin(), user_roles.end(), required_role) != user_roles.end()) {
            return true;
        }
    ...
    EXPECT_FALSE(has_permission(admin_user.roles, "editor")); // Admin doesn't have editor role in our hierarchy
    ...
    auto protect_route = [&auth_manager, &has_permission](const std::string& token, const std::string& required_role) -> bool {
...
TEST(AuthManagerTest, ErrorHandling) {
    ...
    std::unordered_map<std::string, std::chrono::time_point<std::chrono::steady_clock>> lockout_until;
...
TEST(AuthManagerTest, ExtendedEdgeCases) {
    ...
    for (int i = 0; i < 100; i++) {
        large_user.metadata["key_" + std::to_string(i)] = std::string(100, 'X'); // 100 character string
    }
...
TEST(AuthManagerTest, SecurityFeatures) {
    ...
    // Tamper with the token payload (modify the middle section which contains the payload)
    ...
    decoded_payload = std::regex_replace(decoded_payload, std::regex("\"sub\":\"secure-user\""), "\"sub\":\"hacker\"");
    
    // Re-encode the payload
    token_parts[1] = base64url_encode(decoded_payload);
    
    // Reconstruct the token without regenerating the signature
    std::string tampered_token = token_parts[0] + "." + token_parts[1] + "." + token_parts[2];
