...
public:
    TestAsyncHandler(std::string handler_id, TaskExecutor *executor, bool signal_error = false, bool auto_complete_task = true,
    ...
    void
    process(std::shared_ptr<qb::http::Context<MockAsyncSession>> ctx) override {
        if (ctx && ctx->session()) {
        ...
        auto shared_ctx = ctx;
        _executor->addTask([shared_ctx, handler_id = _handler_id, signal_err = _signal_error, auto_complete = _auto_complete_task,
                            check_cancel = _check_cancel_in_task]() {
                ...
                if (shared_ctx->session()) {
                    ...
                    if (pos != std::string::npos) {
                        ...
                        bool end_ok   = ((pos + handle_called_marker.length() == ids_str.length())
                                         || (pos + handle_called_marker.length() < ids_str.length()
                                             && ids_str[pos + handle_called_marker.length()] == ';'));
                        if (start_ok && end_ok) {
                    ...
                    if (ids_str == handler_id
                        || (ids_str.length() > handler_id.length()
                            && ids_str.substr(ids_str.length() - handler_id.length() - 1) == (";" + handler_id))) {
                        ids_str += "_CANCELLED_PRE_LOGIC";
                    } else if (ids_str.empty()) {
                        // Should not be empty if handle was called
            ...
            if (shared_ctx->session()) {
                ...
                if (pos != std::string::npos) {
                    ...
                    if (start_ok && end_ok) {
                        ...
                        if (ids_str.find(current_handler_token) == std::string::npos && ids_str.rfind(current_handler_token_at_start) != 0 &&
                            // Check if it's not already at the start
                            ids_str != handler_id) {
                            // Check if it's not the only ID
                            if (!ids_str.empty()) {
                                ids_str += ";";
                            }
                            ids_str += handler_id;
                    ...
                    bool already_present = (ids_str == handler_id || ids_str.find(current_handler_token_at_start) == 0
...
public:
    TestAsyncMiddleware(std::string id, TaskExecutor *executor, std::string header_name = "X-Async-Middleware",
                        std::string header_value = "Applied", bool signal_error_in_task = false, bool check_cancel_in_task = false)
        : _id(std::move(id))
        , _executor(executor)
    ...
    process(std::shared_ptr<qb::http::Context<MockAsyncSession>> ctx) override {
        ...
        auto shared_ctx = ctx;
        _executor->addTask([shared_ctx, hid = _id, hn = _header_name, hv = _header_value, signal_err_task = _signal_error_in_task,
                            check_cancel_task = _check_cancel_in_task]() {
...
TEST_F(RouterAsyncTest, AsyncRouteWithPathParametersDeferred) {
    ...
    ASSERT_EQ(_task_executor.getPendingTaskCount(), 1) << "Task was not enqueued by TestAsyncHandler into the fixture's TaskExecutor, despite "
                                                          "TestAsyncHandler::handle apparently being called.";
...
TEST_F(RouterAsyncTest, AsyncRouteWithSyncMiddlewareDeferred) {
    router.use([](auto ctx, auto next) {
        ...
        if (ctx->session()) {
            // It's good practice to check for session existence
            if (ctx->session()->_handler_id_executed.empty()) {
    ...
    EXPECT_EQ(mock_session->_response.header("X-Sync-Middleware"), "Applied") << "Header from sync middleware not found after task completion.";
    ASSERT_TRUE(mock_session->_async_handler_logic_done);
    EXPECT_EQ(mock_session->_response.status(), HTTP_STATUS_OK);
...
TEST_F(RouterAsyncTest, AsyncMiddlewareAndAsyncHandler) {
    ...
    _task_executor.processAllTasks();
    // Async Middleware task runs, calls complete(CONTINUE). Handler's process() is called and queues its task.
...
TEST_F(RouterAsyncTest, MixedTaskChain_SyncMw_AsyncMw_AsyncHandler) {
    ...
    router.use([](auto ctx, auto next) {
        ...
        if (ctx->session()) {
            ctx->session()->_handler_id_executed +=
                (ctx->session()->_handler_id_executed.empty() ? "" : ";") + std::string("sync_functional_mw");
        }
        next();
    });
    ...
    EXPECT_EQ(mock_session->_handler_id_executed, "sync_functional_mw") << "After route(): Only sync_functional_mw ID expected.";
    ...
    _task_executor.processAllTasks();
    // Process TestAsyncHandler's lambda task. This calls complete(COMPLETE). Context finalizes.
    ...
    EXPECT_EQ(mock_session->_response.header("X-Async-Test"), "AsyncApplied") << "X-Async-Test header missing or incorrect after all tasks.";
...
TEST_F(RouterAsyncTest, AsyncHandlerCancellation) {
    ...
    auto asyncHandler = std::make_shared<TestAsyncHandler>("async_cancel_handler", &_task_executor, false /*signal_error*/,
                                                           false /*auto_complete_task*/, true /*check_cancel_in_task*/);
...
TEST_F(RouterAsyncTest, CancelDuringAsyncMiddlewareBeforeHandlerTask) {
    ...
    _task_executor.processAllTasks();
    // Async Middleware task runs, calls complete(CONTINUE). Handler's process() is called & queues its task.
...
TEST_F(RouterAsyncTest, CancelContextAlreadyOnErrorStatus) {
    ...
    auto ctx = std::make_shared<qb::http::Context<MockAsyncSession>>(std::move(req), std::move(resp_proto), temp_session, on_finalized_for_test,
...
TEST_F(RouterAsyncTest, AsyncMiddlewareTaskSignalsError) {
    ...
    auto erroringMiddleware = std::make_shared<TestAsyncMiddleware>("error_mw", &_task_executor, "X-Error-Middleware", "ErrorSignalled",
    ...
    EXPECT_EQ(mock_session->_response.status(), HTTP_STATUS_INTERNAL_SERVER_ERROR) << "Context should have error status from middleware.";
...
TEST_F(RouterAsyncTest, AsyncMiddlewareTaskCancellation) {
    ...
    auto cancellableMiddleware =
        std::make_shared<TestAsyncMiddleware>("cancel_check_mw", &_task_executor, "X-Cancel-Check-MW", "NotAppliedDueToCancel",
                                              false /* signal_error_in_task */, true /* check_cancel_in_task */);
...
TEST_F(RouterAsyncTest, NotFoundWithGlobalAsyncMiddleware) {
    ...
    auto globalAsyncMiddleware = std::make_shared<TestAsyncMiddleware>("global_async_mw", &_task_executor, "X-Global-Async-MW", "AppliedGlobal",
                                                                       false /* signal_error_in_task */, false /* check_cancel_in_task */);
    ...
    router.get("/async/lambda", [this](std::shared_ptr<qb::http::Context<MockAsyncSession>> ctx) {
        ...
        _task_executor.addTask([ctx]() {
            ...
            if (ctx && ctx->session()) {
                ...
                if (ids_str == marker) {
                    // Simple replacement if it's the only one
