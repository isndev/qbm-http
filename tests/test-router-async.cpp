...
public:
    TestAsyncHandler(std::string handler_id, TaskExecutor *executor, bool signal_error = false,
                     bool auto_complete_task = true, bool check_cancel_in_task = false)
    ...
    void process(std::shared_ptr<qb::http::Context<MockAsyncSession> > ctx) override {
        ...
        auto shared_ctx = ctx;
        _executor->addTask(
            [shared_ctx, handler_id = _handler_id, signal_err = _signal_error, auto_complete = _auto_complete_task,
                    ...
                    if (shared_ctx->session()) {
                        ...
                        if (pos != std::string::npos) {
                            ...
                            bool end_ok = ((pos + handle_called_marker.length() == ids_str.length()) ||
                                           (pos + handle_called_marker.length() < ids_str.length() && ids_str[
                                                pos + handle_called_marker.length()] == ';'));
                            if (start_ok && end_ok) {
                        ...
                        if (ids_str == handler_id || (ids_str.length() > handler_id.length() && ids_str.substr(
                                                          ids_str.length() - handler_id.length() - 1) == (
                                                          ";" + handler_id))) {
                    ...
                    if (pos != std::string::npos) {
                        ...
                        if (start_ok && end_ok) {
                            ...
                            if (ids_str.find(current_handler_token) == std::string::npos &&
                                ids_str.rfind(current_handler_token_at_start) != 0 &&
...
TEST_F(RouterAsyncTest, AsyncRouteWithPathParametersDeferred) {
    ...
    ASSERT_NE(asyncHandler->getExecutorUsed(),
              nullptr) << "TestAsyncHandler has a null executor pointer after construction.";
    ASSERT_EQ(asyncHandler->getExecutorUsed(),
              &_task_executor) << "TestAsyncHandler is not using the fixture's TaskExecutor instance!";
...
TEST_F(RouterAsyncTest, AsyncMiddlewareAndAsyncHandler) {
    ...
    ASSERT_FALSE(mock_session->_async_handler_logic_done);
    EXPECT_TRUE(
        mock_session->_handler_id_executed.empty(
        )) << "handler_id_executed should be empty before middleware task runs.";
    ...
    ASSERT_EQ(_task_executor.getPendingTaskCount(),
              1) << "Handler task was not enqueued after middleware task completion.";
    ASSERT_FALSE(mock_session->_async_handler_logic_done);
...
TEST_F(RouterAsyncTest, AsyncHandlerSignalsError) {
    ...
    auto asyncHandler = std::make_shared<TestAsyncHandler>("async_error_handler", &_task_executor,
                                                           true /* signal_error */);
...
TEST_F(RouterAsyncTest, MixedTaskChain_SyncMw_AsyncMw_AsyncHandler) {
    ...
    router.use([](auto ctx, auto next) {
        ...
        if (ctx->session()) {
            ctx->session()->_handler_id_executed += (ctx->session()->_handler_id_executed.empty() ? "" : ";") +
                    std::string("sync_functional_mw");
        }
        next();
    });

    // 2. Async Middleware (TestAsyncMiddleware)
    auto asyncMiddleware = std::make_shared<TestAsyncMiddleware>("async_test_mw", &_task_executor, "X-Async-Test",
                                                                 "AsyncApplied");
    router.use(asyncMiddleware);
    ...
    ASSERT_EQ(_task_executor.getPendingTaskCount(),
              1) << "Async middleware's (TestAsyncMiddleware) lambda task not queued.";
    ASSERT_FALSE(mock_session->_async_handler_logic_done);
    ...
    EXPECT_EQ(mock_session->_response.header("X-Sync-Test"),
              "SyncApplied") << "X-Sync-Test header missing after all tasks.";
    EXPECT_EQ(mock_session->_response.header("X-Async-Test"),
              "AsyncApplied") << "X-Async-Test header missing or incorrect after all tasks.";
...
TEST_F(RouterAsyncTest, AsyncHandlerCancellation) {
    ...
    auto asyncHandler = std::make_shared<TestAsyncHandler>("async_cancel_handler", &_task_executor,
                                                           false /*signal_error*/, false /*auto_complete_task*/,
...
TEST_F(RouterAsyncTest, CancelBeforeAnyTasksProcessed) {
    auto asyncHandler = std::make_shared<
        TestAsyncHandler>("async_cancel_immediate", &_task_executor, false, true, true);
...
TEST_F(RouterAsyncTest, CancelDuringAsyncMiddlewareBeforeHandlerTask) {
    ...
    auto asyncHandler = std::make_shared<TestAsyncHandler>("async_handler_after_mw_cancel", &_task_executor, false,
                                                           true, true);
...
TEST_F(RouterAsyncTest, RouteGroupWithAsyncMiddlewareAndHandler) {
    auto groupAsyncMiddleware = std::make_shared<TestAsyncMiddleware>("group_async_mw", &_task_executor,
                                                                      "X-Group-Async-MW");
...
TEST_F(RouterAsyncTest, AsyncMiddlewareTaskCancellation) {
    ...
    ASSERT_FALSE(
        mock_session->
        _async_handler_logic_done) << "Downstream handler logic ran despite middleware task cancellation.";
    ASSERT_EQ(_task_executor.getPendingTaskCount(),
              0) << "No further tasks should be queued if middleware task cancelled.";
}
