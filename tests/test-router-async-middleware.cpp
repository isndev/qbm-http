/**
 * @file test-router-async-middleware.cpp
 * @brief Tests for asynchronous middleware in the HTTP router
 */
...
struct MockSession {
    ...
    int responseCount() const { return _all_responses.size(); }
    ...
    MockSession& operator<<(qb::http::Response const& response) {
        ...
        _response = qb::http::Response(response);
        _all_responses.push_back(_response);
        return *this;
    }
    
    // Add method to simulate a connected session
    bool is_connected() const { return !_closed; }
...
using AsyncCompletionHandler = qb::http::AsyncCompletionHandler<MockSession, std::string>;
...
protected:
    ...
    TestRequest createRequest(http_method method, std::string path) {
        TestRequest req;
        req.method = method;
    ...
    void simulateAsyncDelay(Func callback, int delay_ms = 10) {
...
TEST_F(RouterAsyncMiddlewareTest, BasicAsyncMiddlewareChaining) {
    ...
    router->use_async([&](Context& ctx, auto next) {
        ...
        auto state_exec_order = std::make_shared<std::vector<std::string>*>(&execution_order);
    ...
    // Test 1: Request with valid auth header
    {
        ...
        EXPECT_EQ(session->_response.body().as<std::string>(), "Resource for user: user123");
    ...
    // Test 2: Request without auth header (should break middleware chain)
    {
        ...
        EXPECT_EQ(session->_response.body().as<std::string>(), "Authentication required");
...
TEST_F(RouterAsyncMiddlewareTest, AsyncMiddlewareWithErrorHandling) {
    ...
    router->use_async([&](Context& ctx, auto next) {
        ...
        simulateAsyncDelay([state_exec_order, ctx_ptr, &ctx, should_fail, next]() mutable {
            if (should_fail) {
    ...
    router->onError(HTTP_STATUS_INTERNAL_SERVER_ERROR, [&](Context& ctx) {
...
TEST_F(RouterAsyncMiddlewareTest, AsyncMiddlewareWithDeferredProcessing) {
    ...
    // Test 3: Long deferral - custom response, no route handler
    {
        ...
        EXPECT_EQ(session->responseCount(), 1);
        EXPECT_EQ(session->_response.status_code, HTTP_STATUS_OK);
        EXPECT_EQ(session->_response.body().as<std::string>(), "Response after long deferral");
...
TEST_F(RouterAsyncMiddlewareTest, AsyncMiddlewareWithRouteParameters) {
    ...
    // Test parameter access in middleware and handler
    {
        ...
        EXPECT_EQ(execution_order[2], "handler_user_id: 123"); // Parameters available in handler
...
TEST_F(RouterAsyncMiddlewareTest, AsyncMiddlewarePriority) {
    ...
    router->GET("/priority", [&](Context& ctx) {
        ...
        if (is_high_priority) {
            ...
            auto execution_order_ptr = std::make_shared<std::vector<std::string>*>(&execution_order);
...
TEST_F(RouterAsyncMiddlewareTest, AsyncMiddlewareParametersSerialization) {
    ...
    EXPECT_EQ(session->responseCount(), 1);
    EXPECT_EQ(session->_response.status_code, HTTP_STATUS_OK);
    EXPECT_EQ(session->_response.body().as<std::string>(), "User: 123, Middleware data: context_value");
}
...
TEST_F(RouterAsyncMiddlewareTest, AsyncMiddlewareErrorRecovery) {
    ...
    router->use_async([&](Context& ctx, auto next) {
        ...
        simulateAsyncDelay([state_exec_order, ctx_ptr, &ctx, trigger_error, next]() mutable {
    ...
    router->use_async([&](Context& ctx, auto next) {
        ...
        simulateAsyncDelay([state_exec_order, ctx_ptr, &ctx, error_code, error_message, next]() mutable {
    ...
    // Test 1: Normal operation, no errors
    {
        ...
        EXPECT_EQ(session->responseCount(), 1);
        EXPECT_EQ(session->_response.status_code, HTTP_STATUS_OK);
        EXPECT_EQ(session->_response.body().as<std::string>(), "Normal processing, no errors");
...
TEST_F(RouterAsyncMiddlewareTest, NestedAsyncOperations) {
    ...
    router->use_async([&](Context& ctx, auto next) {
        ...
        simulateAsyncDelay([this, state_exec_order, ctx_ptr, &ctx, nested_data_1, nested_data_2, next_cb]() mutable {
            ...
            ctx.response.add_header("X-Data-Summary", nested_data_1 + "+" + nested_data_2);
    ...
    router->GET("/nested-async", [&](Context& ctx) {
        ...
        ctx.response.status_code = HTTP_STATUS_OK;
        ctx.response.body() = "Nested operations completed successfully: " + 
    ...
    // Test the nested async operations
    {
        ...
        EXPECT_TRUE(std::find(execution_order.begin(), execution_order.end(), 
                    "all_nested_operations_complete") != execution_order.end());
        EXPECT_TRUE(std::find(execution_order.begin(), execution_order.end(), 
        ...
        EXPECT_EQ(session->_response.header("X-Data-Summary"), "nested_value_1+nested_value_2");
    }
}
...
TEST_F(RouterAsyncMiddlewareTest, AsyncMiddlewareTimeoutHandling) {
    ...
    router->use_async([&](Context& ctx, auto next) {
        ...
        if (timeout_behavior == "normal") {
            ...
            simulateAsyncDelay([state_exec_order, ctx_ptr, &ctx, next_cb]() mutable {
                (*state_exec_order)->push_back("timeout_middleware_normal_completion");
        ...
        if (timeout_behavior == "timeout") {
            ...
            // In a real scenario, a timeout monitor would detect this and cancel the operation
            simulateAsyncDelay([state_exec_order, ctx_ptr]() mutable {
                (*state_exec_order)->push_back("timeout_middleware_simulated_timeout");
        ...
        if (timeout_behavior == "cancel") {
            ...
            simulateAsyncDelay([state_exec_order, ctx_ptr, was_cancelled]() mutable {
                if (*was_cancelled) {
            ...
            simulateAsyncDelay([state_exec_order, ctx_ptr, &ctx, next_cb, was_cancelled]() mutable {
    ...
    // Test 1: Non-timed operation (immediate)
    {
        ...
        EXPECT_EQ(session->responseCount(), 1);
        EXPECT_EQ(session->_response.status_code, HTTP_STATUS_OK);
        EXPECT_EQ(session->_response.body().as<std::string>(), "Normal operation completed");
    }
    ...
    // Test 2: Normal completion (no timeout)
    {
        ...
        EXPECT_TRUE(std::find(execution_order.begin(), execution_order.end(), 
                    "timeout_middleware_normal_completion") != execution_order.end());
        EXPECT_TRUE(std::find(execution_order.begin(), execution_order.end(), 
                    "route_handler") != execution_order.end());
    ...
    // Test 3: Operation timeout
    {
        ...
        EXPECT_TRUE(std::find(execution_order.begin(), execution_order.end(), 
                    "timeout_middleware_simulated_timeout") != execution_order.end());
        EXPECT_TRUE(std::find(execution_order.begin(), execution_order.end(), 
    ...
    // Test 4: Operation cancellation
    {
        ...
        EXPECT_EQ(session->_response.body().as<std::string>(), "Operation cancelled");
...
TEST_F(RouterAsyncMiddlewareTest, ParallelAsyncOperations) {
    ...
    router->use_async([&](Context& ctx, auto next) {
        ...
        simulateAsyncDelay([state_exec_order, ctx_ptr, &ctx, next_cb, parallel_ops_completed, expected_ops]() mutable {
    ...
    // Test parallel operations
    {
        ...
        EXPECT_TRUE(std::find(execution_order.begin(), execution_order.end(), 
                    "all_parallel_operations_complete") != execution_order.end());
        
        // Results middleware should have processed the data
        EXPECT_TRUE(std::find(execution_order.begin(), execution_order.end(), 
                    "results_middleware_start") != execution_order.end());
        EXPECT_TRUE(std::find(execution_order.begin(), execution_order.end(), 
                    "results_middleware_data1: operation_1_data") != execution_order.end());
        EXPECT_TRUE(std::find(execution_order.begin(), execution_order.end(), 
                    "results_middleware_data2: operation_2_data") != execution_order.end());
        ...
        EXPECT_EQ(session->_response.header("X-Combined-Result"), "operation_1_data+operation_2_data");
    }
}
...
TEST_F(RouterAsyncMiddlewareTest, AsyncMiddlewareCancellation) {
    ...
    router->use_async([&](Context& ctx, auto next) {
        ...
        if (behavior == "cancel") {
            ...
            // The main operation
            simulateAsyncDelay([state_exec_order, operation_cancelled]() mutable {
            ...
            simulateAsyncDelay([state_exec_order, ctx_ptr, &ctx, next_cb, operation_cancelled]() mutable {
    ...
    // Test 1: Normal operation (no cancellation)
    {
        ...
        EXPECT_EQ(session->responseCount(), 1);
        EXPECT_EQ(session->_response.status_code, HTTP_STATUS_OK);
        EXPECT_EQ(session->_response.body().as<std::string>(), "Operation completed normally");
    }
    ...
    // Test 2: Cancelled operation
    {
        ...
        EXPECT_EQ(session->responseCount(), 1);
        EXPECT_EQ(session->_response.status_code, HTTP_STATUS_CONFLICT);
        EXPECT_EQ(session->_response.body().as<std::string>(), "Operation was cancelled");
...
TEST_F(RouterAsyncMiddlewareTest, AsyncMiddlewareEarlyReturnsAndCancellation) {
    ...
    router->use_async([&](Context& ctx, auto next) {
        ...
        // First parallel operation
        simulateAsyncDelay([this, cancelled, operations_completed, total_operations, &ctx, next]() {
            if (*cancelled) {
        ...
        simulateAsyncDelay([this, cancelled, operations_completed, total_operations, &ctx, next]() {
            ...
            if (ctx.request.header("X-Trigger-Failure") == "true") {
                ...
                *cancelled = true;
                ...
                ctx.response.body() = "Operation 2 failed, cancelling all operations";
    ...
    // Test 1: All operations complete successfully
    {
        ...
        EXPECT_EQ(session->responseCount(), 1);
        EXPECT_EQ(session->_response.status_code, HTTP_STATUS_OK);
        EXPECT_EQ(session->_response.body().as<std::string>(), "All operations completed successfully");
        EXPECT_EQ(session->_response.header("X-Second-Middleware"), "executed");
    }
    ...
    // Test 2: Operation 2 fails and triggers cancellation
    {
        ...
        // Check for cancellation effects - this may or may not happen depending on timing
        // so we don't assert on it directly
        bool op1_skipped = std::find(execution_order.begin(), execution_order.end(), 
                                    "operation_1_skipped_due_to_cancellation") != execution_order.end();
        bool op3_skipped = std::find(execution_order.begin(), execution_order.end(), 
                                    "operation_3_skipped_due_to_cancellation") != execution_order.end();
        
        // Log the cancellation state but don't assert on it
        std::cout << "Operation 1 skipped: " << (op1_skipped ? "YES" : "NO") << std::endl;
        std::cout << "Operation 3 skipped: " << (op3_skipped ? "YES" : "NO") << std::endl;
        ...
        EXPECT_EQ(session->_response.body().as<std::string>(), "Operation 2 failed, cancelling all operations");
