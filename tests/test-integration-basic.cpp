      ...
      : qb::http::use<BasicIntegrationServer>::server<BasicIntegrationSession>()
    {
        ...
        router().get("/async", [](std::shared_ptr<SessionContext> ctx) {
            ...
            qb::io::async::callback([ctx_capture = ctx]() {
                ctx_capture->response().status_code = HTTP_STATUS_OK;
...
protected:
    ...
    void TearDown() override {
        ...
        if (expected_server_assertions > 0) { // Only check if assertions were expected
    ...
    void MakeClientRequest(Func&& client_logic, int expected_server_req_increment = 1, int expected_server_assert_increment = 0) {
        if (!server_ready) {
...
TEST_F(HttpBasicIntegrationTest, PostRequest) {
    ...
    MakeClientRequest([] {
        ...
        qb::http::Request request{qb::http::method::HTTP_POST, {"http://localhost:9876/test"}};
...
TEST_F(HttpBasicIntegrationTest, PutRequestWithParam) {
    ...
    MakeClientRequest([] {
        ...
        qb::http::Request request{qb::http::method::HTTP_PUT, {"http://localhost:9876/test/123"}};
...
TEST_F(HttpBasicIntegrationTest, DeleteRequestWithParam) {
    ...
    MakeClientRequest([] {
        ...
        qb::http::Request request{qb::http::method::HTTP_DELETE, {"http://localhost:9876/test/456"}};
