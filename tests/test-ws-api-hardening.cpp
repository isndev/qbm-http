...
class SinkServerClient : public qb::io::use<SinkServerClient>::tcp::client<SinkServer> {
public:
    explicit SinkServerClient(IOServer &server)
        : client(server) {}
};
...
class HandshakeValidationServerClient : public qb::io::use<HandshakeValidationServerClient>::tcp::client<HandshakeValidationServer> {
public:
    ...
    explicit HandshakeValidationServerClient(IOServer &server)
        : client(server) {}
...
class HandshakeValidationServer : public qb::io::use<HandshakeValidationServer>::tcp::server<HandshakeValidationServerClient> {};
...
class ControlProbeServerClient : public qb::io::use<ControlProbeServerClient>::tcp::client<ControlProbeServer> {
public:
    using Protocol    = qb::http::protocol<ControlProbeServerClient>;
    using WS_Protocol = qb::http::ws::protocol<ControlProbeServerClient>;

    explicit ControlProbeServerClient(IOServer &server)
        : client(server) {}
...
class ControlProbeServer : public qb::io::use<ControlProbeServer>::tcp::server<ControlProbeServerClient> {};

class BadSubprotocolServer;

class BadSubprotocolServerClient : public qb::io::use<BadSubprotocolServerClient>::tcp::client<BadSubprotocolServer> {
public:
    ...
    explicit BadSubprotocolServerClient(IOServer &server)
        : client(server) {}
...
class BadSubprotocolServer : public qb::io::use<BadSubprotocolServer>::tcp::server<BadSubprotocolServerClient> {};

class BadConnectionTokenServer;

class BadConnectionTokenServerClient : public qb::io::use<BadConnectionTokenServerClient>::tcp::client<BadConnectionTokenServer> {
public:
    ...
    explicit BadConnectionTokenServerClient(IOServer &server)
        : client(server) {}
...
class BadConnectionTokenServer : public qb::io::use<BadConnectionTokenServer>::tcp::server<BadConnectionTokenServerClient> {};

class MaskedFrameServer;

class MaskedFrameServerClient : public qb::io::use<MaskedFrameServerClient>::tcp::client<MaskedFrameServer> {
public:
    using Protocol    = qb::http::protocol<MaskedFrameServerClient>;
    using WS_Protocol = qb::http::ws::protocol<MaskedFrameServerClient>;

    explicit MaskedFrameServerClient(IOServer &server)
        : client(server) {}
...
class MaskedFrameServer : public qb::io::use<MaskedFrameServer>::tcp::server<MaskedFrameServerClient> {};

class ReservedOpcodeServer;

class ReservedOpcodeServerClient : public qb::io::use<ReservedOpcodeServerClient>::tcp::client<ReservedOpcodeServer> {
public:
    ...
    explicit ReservedOpcodeServerClient(IOServer &server)
        : client(server) {}
...
class ReservedOpcodeServer : public qb::io::use<ReservedOpcodeServer>::tcp::server<ReservedOpcodeServerClient> {};

class InvalidClosePayloadServer;

class InvalidClosePayloadServerClient : public qb::io::use<InvalidClosePayloadServerClient>::tcp::client<InvalidClosePayloadServer> {
public:
    ...
    explicit InvalidClosePayloadServerClient(IOServer &server)
        : client(server) {}
    ...
    on(Protocol::request &&request) {
        ...
        *this << response;
        ...
        const std::array<char, 3> frame{{static_cast<char>(0x88u), 0x01, static_cast<char>(0x00)}};
...
class InvalidClosePayloadServer : public qb::io::use<InvalidClosePayloadServer>::tcp::server<InvalidClosePayloadServerClient> {};

class InvalidCloseReasonUtf8Server;

class InvalidCloseReasonUtf8ServerClient : public qb::io::use<InvalidCloseReasonUtf8ServerClient>::tcp::client<InvalidCloseReasonUtf8Server> {
public:
    ...
    explicit InvalidCloseReasonUtf8ServerClient(IOServer &server)
        : client(server) {}
    ...
    on(Protocol::request &&request) {
        ...
        *this << response;
        ...
        const std::array<char, 7> frame{
            {static_cast<char>(0x88u), 0x05, static_cast<char>(0x03), static_cast<char>(0xE8), static_cast<char>(0xED), static_cast<char>(0xA0),
...
class InvalidCloseReasonUtf8Server : public qb::io::use<InvalidCloseReasonUtf8Server>::tcp::server<InvalidCloseReasonUtf8ServerClient> {};
...
public:
    ...
    explicit CloseEchoProbeServerClient(IOServer &server)
        : client(server) {}
...
class CloseEchoProbeServer : public qb::io::use<CloseEchoProbeServer>::tcp::server<CloseEchoProbeServerClient> {};

bool
run_until(const std::function<bool()> &condition, int max_iterations = 300, int sleep_ms = 5) {
...
TEST(WebSocketApiHardening, ClientControlFramesAreAcceptedAsMaskedByServerParser) {
    ...
    ASSERT_TRUE(run_until([&]() { return g_control_ping_received > 0 || disconnected; })) << "No control-frame activity observed";
...
TEST(WebSocketApiHardening, ServerRejectsMalformedSecWebSocketKey) {
    ...
    if (!response.empty()) {
        EXPECT_NE(response.find("400"), std::string::npos) << "Expected a BAD_REQUEST style response, got:\n" << response;
    }
}
...
TEST(WebSocketApiHardening, HandshakeHostHeaderIncludesNonDefaultPortAndIpv6Brackets) {
    ...
    qb::http::ws::client client;
    client.on_sending_http_request([&captured_host](auto &event) { captured_host = std::string(event.request.header("host")); });

    client.connect(qb::io::uri("ws://[::1]:20127/path"), 1000ms);

    ASSERT_TRUE(run_until([&captured_host]() { return !captured_host.empty(); })) << "WebSocket client did not reach request emission phase";
...
TEST(WebSocketApiHardening, ClientRejectsServerSubprotocolNotOfferedByClient) {
    ...
    ASSERT_TRUE(run_until([&]() { return errored || connected || disconnected; })) << "WebSocket client produced no terminal signal";
...
TEST(WebSocketApiHardening, ClientRejectsReservedOpcodeFromServer) {
    ...
    ASSERT_TRUE(run_until([&]() { return errored || disconnected || messages > 0; })) << "Client did not react to reserved opcode frame";
...
TEST(WebSocketApiHardening, ClientRejectsInvalidClosePayloadLengthOne) {
    ...
    ASSERT_TRUE(run_until([&]() { return errored || disconnected || closes > 0; })) << "Client did not react to invalid close payload";
...
TEST(WebSocketApiHardening, ClientRejectsInvalidUtf8InCloseReason) {
    ...
    ASSERT_TRUE(run_until([&]() { return errored || disconnected || closes > 0; })) << "Client did not react to invalid UTF-8 close reason";
...
TEST(WebSocketApiHardening, ClientEchoesPeerCloseFrame) {
    ...
    ASSERT_TRUE(run_until([&]() { return disconnected || g_close_echo_received > 0; }, 600, 5))
        << "Client never surfaced a close/disconnect signal";
