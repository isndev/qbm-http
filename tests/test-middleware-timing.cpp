...
protected:
    ...
    qb::http::RouteHandlerFn<MockTimingSession> simple_handler(
...
TEST_F(TimingMiddlewareTest, CustomNaming) {
    auto timing_mw_custom_name = qb::http::timing_middleware<MockTimingSession>(
        _test_timing_callback, "MyCustomTimerName");
...
TEST_F(TimingMiddlewareTest, TimingWhenHandlerThrowsException) {
    ...
    auto error_task = std::make_shared<qb::http::MiddlewareTask<MockTimingSession> >(
        error_finalizer_mw, "SimpleErrorFinalizerTask");
