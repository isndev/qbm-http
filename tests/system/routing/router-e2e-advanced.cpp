...
public:
    ...
    RouterE2EServer() {
        ...
        qb::http::RouteHandlerFn<RouterE2ESession> error_handler =
        ...
        auto error_task = std::make_shared<qb::http::RouteLambdaTask<RouterE2ESession>>(
            error_handler, "GlobalErrorHandlerTask");
...
TEST_F(RouterE2ETest, PathParamSurvivesWire) {
    auto reply =
        qb::http::run_sync(qb::http::GET(qb::http::Request{{url("/param/hello")}}, kClientTimeout));
...
TEST_F(RouterE2ETest, UrlEncodedPathParamDecodesServerSide) {
    ...
    const std::vector<Case> cases = {
        {"simplevalue"}, {"hello world"}, {"path/component-ish"}, {"!@#$%^&*()"}};

    for (const auto &c : cases) {
        const auto path = "/param/" + url_encode(c.raw);
        auto       reply =
            qb::http::run_sync(qb::http::GET(qb::http::Request{{url(path)}}, kClientTimeout));
        EXPECT_EQ(reply.response.status(), qb::http::status::OK) << "raw=" << c.raw;
        EXPECT_EQ(reply.response.body().template as<std::string>(), "param:" + c.raw)
            << "raw=" << c.raw;
    }
}
...
TEST_F(RouterE2ETest, WildcardCaptureSurvivesWire) {
    auto reply = qb::http::run_sync(
        qb::http::GET(qb::http::Request{{url("/files/some/long/path/to/file.txt")}},
                      kClientTimeout));
    EXPECT_EQ(reply.response.status(), qb::http::status::OK);
    EXPECT_EQ(reply.response.body().template as<std::string>(),
              "file:some/long/path/to/file.txt");
}

TEST_F(RouterE2ETest, QueryStringParsedFromWire) {
    auto with_query = qb::http::run_sync(
        qb::http::GET(qb::http::Request{{url("/search?q=widgets&page=3")}}, kClientTimeout));
    ...
    auto defaults =
        qb::http::run_sync(qb::http::GET(qb::http::Request{{url("/search")}}, kClientTimeout));
...
TEST_F(RouterE2ETest, HeadResponseHasNoBodyOnTheWire) {
    auto reply = qb::http::run_sync(
        qb::http::HEAD(qb::http::Request{qb::http::method::HEAD, {url("/ping")}}, kClientTimeout));
    EXPECT_EQ(reply.response.status(), qb::http::status::OK);
...
TEST_F(RouterE2ETest, OptionsSerializesExplicitAllowHeader) {
    auto reply = qb::http::run_sync(
        qb::http::OPTIONS(qb::http::Request{qb::http::method::OPTIONS, {url("/resource")}},
                          kClientTimeout));
...
TEST_F(RouterE2ETest, AsyncHandlerCompletesOverTheLoop) {
    auto reply =
        qb::http::run_sync(qb::http::GET(qb::http::Request{{url("/async")}}, kClientTimeout));
    EXPECT_EQ(reply.response.status(), qb::http::status::OK);
...
TEST_F(RouterE2ETest, MiddlewareInducedErrorYieldsExact503) {
    auto reply = qb::http::run_sync(
        qb::http::GET(qb::http::Request{{url("/mw-error/route")}}, kClientTimeout));
    ...
    qb::io::async::run_sync([&]() -> qb::io::async::task<void> {
        for (int i = 0; i < kN; ++i) {
            auto reply = co_await qb::http::GET(
                qb::http::Request{{url("/param/req" + std::to_string(i))}}, kClientTimeout);
