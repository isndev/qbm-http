...
public:
    Http2IntegrationServer()
        ...
        router().get("/api/reset-stream", [](auto ctx) {
            ...
            if (session
                && session->reset_stream(static_cast<uint32_t>(ctx->request().stream_id), qb::protocol::http2::ErrorCode::CANCEL,
...
public:
    ...
    void
    MakeHttp2ClientRequest(Func &&client_logic, int expected_server_req_increment = 1, int expected_server_assert_increment = 0) {
    ...
    MakeHttp2ClientRequest(
        [&]() {
            ...
            ASSERT_TRUE(response_received.load(std::memory_order_acquire))
                << "Client did not receive response for SimpleGetRequest within timeout.";
...
TEST_F(Http2ClientIntegrationTest, PostRequestWithData) {
    MakeHttp2ClientRequest(
        [&]() {
            ...
            ASSERT_TRUE(response_received.load(std::memory_order_acquire))
                << "Client did not receive response for PostRequestWithData within timeout.";
        },
        1, 1);
}
...
TEST_F(Http2ClientIntegrationTest, ConcurrentRequests) {
    MakeHttp2ClientRequest(
        [&]() {
            ...
            ASSERT_EQ(responses_received.load(std::memory_order_acquire), 3)
                << "Client did not receive all responses for ConcurrentRequests within timeout.";
        },
        3, 3);
}
...
TEST_F(Http2ClientIntegrationTest, ErrorHandling) {
    MakeHttp2ClientRequest(
        [&]() {
            ...
            ASSERT_TRUE(response_received.load(std::memory_order_acquire))
                << "Client did not receive response for ErrorHandling within timeout.";
        },
        1, 0);
}
