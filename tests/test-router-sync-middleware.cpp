/**
 * @file test-router-sync-middleware.cpp
 * @brief Tests for synchronous middleware in the HTTP router
 */
...
TEST_F(RouterSyncMiddlewareTest, MiddlewareChainBreaking) {
    ...
    // Test 2: Invalid auth token - should break at auth middleware
    {
        ...
        EXPECT_EQ(session->_response.body().as<std::string>(), "Invalid authentication token");
    ...
    // Test 3: Valid token but insufficient permissions - should break at permission middleware
    {
        execution_order.clear();
        session->reset();
    ...
    // Test 4: Valid token and sufficient permissions - complete chain
    {
        ...
        EXPECT_TRUE(router->route(*session, req));
        EXPECT_EQ(session->_response.status_code, HTTP_STATUS_OK);
        EXPECT_EQ(session->_response.body().as<std::string>(), "Protected content for user: user-123");
...
TEST_F(RouterSyncMiddlewareTest, ContentTransformation) {
    ...
    router->GET("/user", [](Context& ctx) {
        ...
        std::string json = R"({"id": 123, "name": "john doe", "email": "john@example.com"})";
...
TEST_F(RouterSyncMiddlewareTest, ConditionalMiddleware) {
    ...
    // Test public route - should only execute global middleware
    {
        ...
        EXPECT_EQ(session->_response.header("X-API-Version"), ""); // Header should not be set
...
TEST_F(RouterSyncMiddlewareTest, ErrorHandlingMiddleware) {
    ...
    router->onError(HTTP_STATUS_INTERNAL_SERVER_ERROR, [&execution_order](Context& ctx) {
    ...
    // Test 2: Error request
    {
        ...
        EXPECT_EQ(session->_response.body().as<std::string>(), "Middleware error occurred");
        ...
        // Note: this implementation may not show global_error_handler depending on router implementation
...
TEST_F(RouterSyncMiddlewareTest, MiddlewareWithURLParameters) {
    ...
    router->GET("/users/:user_id/profile", [&handler_had_params, &handler_user_id](Context& ctx) {
    ...
    // Test: Access to parameters in middleware vs handler
    {
        ...
        std::cout << "Parameters accessible in middleware: " << (middleware_had_params ? "yes" : "no") << std::endl;
    ...
    // Test 2: Workaround - Using a route-specific middleware pattern
    {
        ...
        router->GET("/api/:version/items/:item_id", [&validation_ran, &version_param, &item_id_param](Context& ctx) {
            // This runs AFTER parameters have been extracted, so we have access to them here
            ...
            if (item_id_param.empty() || !std::all_of(item_id_param.begin(), item_id_param.end(), ::isdigit)) {
                ctx.response.status_code = HTTP_STATUS_BAD_REQUEST;
        ...
        // Test the workaround with invalid parameters
        {
            ...
            EXPECT_EQ(session->_response.body().as<std::string>(), "Invalid API version");
