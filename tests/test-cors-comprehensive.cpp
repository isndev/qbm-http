...
TEST_F(CorsComprehensiveTest, NonAllowedOriginFallback) {
    ...
    router->enable_cors(CorsOptions().origins({"https://app.example.com", "https://admin.example.com"}));
...
TEST_F(CorsComprehensiveTest, CorsPerformance) {
    ...
    auto req = createRequest(HTTP_GET, "/api/performance", "https://subdomain50.example.com");
    ...
    EXPECT_EQ(session.header("Access-Control-Allow-Origin"), "https://subdomain50.example.com");
    EXPECT_EQ(session.response().status_code, HTTP_STATUS_OK);
    ...
    std::cout << "CORS processing time: " << duration.count() << " microseconds" << std::endl;
}
...
TEST_F(CorsComprehensiveTest, CombinedMatchingStrategies) {
    ...
    auto combined_matcher = [](const std::string& origin) -> bool {
        ...
        if (origin.find("https://internal-") == 0 && origin.find(".corp.example.com") != std::string::npos) {
            // Extraire l'ID de l'équipe pour des vérifications supplémentaires si nécessaire
    ...
    router->route(session, req2);
    EXPECT_EQ(session.header("Access-Control-Allow-Origin"), "https://user.api.example.com");
    session.reset();
    ...
    EXPECT_EQ(session.header("Access-Control-Allow-Origin"), "https://shard-123.example.com");
    session.reset();
    ...
    auto req5 = createRequest(HTTP_GET, "/api/users", "https://internal-devteam.corp.example.com");
    router->route(session, req5);
    EXPECT_EQ(session.header("Access-Control-Allow-Origin"), "https://internal-devteam.corp.example.com");
    session.reset();
...
TEST_F(CorsComprehensiveTest, OriginEscaping) {
    ...
    auto req1 = createRequest(HTTP_GET, "/api/users", "https://sub.example.com/with%20space");
    router->route(session, req1);
    EXPECT_EQ(session.header("Access-Control-Allow-Origin"), "https://sub.example.com/with%20space");
    session.reset();
    
    // Test avec une origine contenant des paramètres de requête
    auto req2 = createRequest(HTTP_GET, "/api/users", "https://app.example.com/path?query=value&param=test");
    router->route(session, req2);
    EXPECT_EQ(session.header("Access-Control-Allow-Origin"), "https://app.example.com/path?query=value&param=test");
    session.reset();
...
TEST_F(CorsComprehensiveTest, WildcardSubdomainMatching) {
    ...
    router->route(session, req2);
    EXPECT_EQ(session.header("Access-Control-Allow-Origin"), "https://dev.api.example.com");
    session.reset();
    ...
    auto req5 = createRequest(HTTP_GET, "/api/users", "https://malicious-example.com.evil.org");
    router->route(session, req5);
    EXPECT_EQ(session.header("Access-Control-Allow-Origin"), "");
}
...
TEST_F(CorsComprehensiveTest, StandardHttpMethodsTest) {
    ...
    auto req1 = createPreflightRequest("/api/users/patch/123", "https://app.example.com", "GET");
    router->route(session, req1);
    
    // Vérifier que la méthode GET est autorisée
    EXPECT_TRUE(session.header("Access-Control-Allow-Methods").find("GET") != std::string::npos);
    EXPECT_EQ(session.response().status_code, HTTP_STATUS_NO_CONTENT);
    session.reset();
    
    // Test de preflight pour HEAD (une méthode moins utilisée)
    auto req2 = createPreflightRequest("/api/users/123", "https://app.example.com", "HEAD");
    router->route(session, req2);
    
    // Vérifier que la méthode HEAD est autorisée
    EXPECT_TRUE(session.header("Access-Control-Allow-Methods").find("HEAD") != std::string::npos);
    EXPECT_EQ(session.response().status_code, HTTP_STATUS_NO_CONTENT);
    session.reset();
    ...
    std::cout << "CORS Debug - Testing actual GET request with Origin: " << req3.header("Origin") << std::endl;
    
    router->route(session, req3);
    
    std::cout << "MockSession received response for actual GET request" << std::endl;
    std::cout << "Captured Access-Control-Allow-Origin: " << session.header("Access-Control-Allow-Origin") << std::endl;
    std::cout << "Captured response status code: " << session.response().status_code << std::endl;
...
TEST_F(CorsComprehensiveTest, PreflightCachingTest) {
    ...
    auto req = createPreflightRequest("/api/users", "https://app.example.com", "POST", {"Content-Type", "Authorization"});
    router->route(session, req);
    ...
    EXPECT_EQ(session.header("Access-Control-Allow-Origin"), "https://app.example.com");
    EXPECT_TRUE(session.header("Access-Control-Allow-Methods").find("POST") != std::string::npos);
    EXPECT_TRUE(session.header("Access-Control-Allow-Headers").find("Content-Type") != std::string::npos);
    EXPECT_TRUE(session.header("Access-Control-Allow-Headers").find("Authorization") != std::string::npos);
}
...
TEST_F(CorsComprehensiveTest, VaryHeaderTest) {
    ...
    auto req2 = createPreflightRequest("/api/users", "https://app.example.com", "POST", {"Content-Type"});
    router->route(session, req2);
    ...
    EXPECT_TRUE(session.header("Vary").find("Access-Control-Request-Headers") != std::string::npos);
}
...
TEST_F(CorsComprehensiveTest, ErrorHandlingTest) {
    ...
    auto safe_matcher = [potentially_failing_matcher](const std::string& origin) -> bool {
        try {
    ...
    auto req1 = createRequest(HTTP_GET, "/api/users", "https://trigger-error.example.com");
    ...
    router->route(session, req2);
    EXPECT_EQ(session.header("Access-Control-Allow-Origin"), "https://valid.example.com");
}
