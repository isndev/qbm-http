        ...
        public:
            GetAllPosts()
                : Route("/?", [](auto &ctx) {
                    msg_count_server_side++;
                    ctx.response.body() =
                        "all posts with offset=" + ctx.request.query("offset");
...
class HttpSecureSession
    : public qb::http::use<HttpSecureSession>::ssl::session<HttpSecureServer> {
public:
...
class HttpSessionView
    : public qb::http::use<HttpSessionView>::session_view<HttpServerView> {
public:
