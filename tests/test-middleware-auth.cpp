...
protected:
    ...
    qb::http::RouteHandlerFn<MockAuthSession>
    success_handler() {
    ...
    void
    configure_router_with_auth_mw(std::shared_ptr<qb::http::AuthMiddleware<MockAuthSession>> auth_mw_to_use) {
    ...
    std::string
    generate_test_token(const qb::http::auth::User &user, const std::string &secret_override = "") {
...
TEST_F(AuthMiddlewareTest, TokenWithoutSubjectOrUsernameIsRejected) {
    ...
    auto req = create_request();
    req.set_header(_auth_options.get_auth_header_name(), _auth_options.get_auth_scheme() + " " + identity_less_token);
    make_request(std::move(req));
...
TEST_F(AuthMiddlewareTest, SignatureVerificationCanBeDisabledWithoutBypassingClaimChecks) {
    ...
    expired_req.set_header(relaxed_options.get_auth_header_name(), relaxed_options.get_auth_scheme() + " " + expired_token);
...
TEST_F(AuthMiddlewareTest, SignatureDisabledInvalidExpClaimIsRejectedWithoutThrowing) {
    ...
    auto req = create_request();
    req.set_header(relaxed_options.get_auth_header_name(), relaxed_options.get_auth_scheme() + " " + malformed_token);
...
TEST_F(AuthMiddlewareTest, RoleMiddlewareUsesPreAuthenticatedContextUser) {
    ...
    _router->use(std::make_shared<qb::http::FunctionalMiddleware<MockAuthSession>>(
        [](auto ctx, auto next_fn) {
            ctx->template set<qb::http::auth::User>("user", qb::http::auth::User{"ctx-user", "contextuser", {"admin"}});
            next_fn();
        },
...
TEST_F(AuthMiddlewareTest, ExpiredTokenMiddleware) {
    ...
    req.set_header(_auth_options.get_auth_header_name(),
                   // Use original _auth_options for header name/scheme consistency if needed, or expiring_options
...
TEST_F(AuthMiddlewareTest, NotYetValidTokenMiddleware) {
    ...
    if (secret_vec.empty()
        && (jwt_create_opts.algorithm == qb::jwt::Algorithm::HS256 || jwt_create_opts.algorithm == qb::jwt::Algorithm::HS384
            || jwt_create_opts.algorithm == qb::jwt::Algorithm::HS512)) {
    ...
    req.set_header(current_auth_opts.get_auth_header_name(), current_auth_opts.get_auth_scheme() + " " + token_not_yet_valid);
    make_request(std::move(req));
...
const char *TEST_RSA_PRIVATE_KEY_PEM = "-----BEGIN PRIVATE KEY-----\n"
...
const char *TEST_RSA_PUBLIC_KEY_PEM = "-----BEGIN PUBLIC KEY-----\n"
...
TEST_F(AuthMiddlewareTest, RSAAlgorithmTokenVerification) {
    ...
    req.set_header(rsa_verify_options.get_auth_header_name(), rsa_verify_options.get_auth_scheme() + " " + rsa_token);
    make_request(std::move(req));
    ...
    EXPECT_EQ(_session->_response.status(), qb::http::status::UNAUTHORIZED) << "RSA token should not validate with HMAC configuration.";
    EXPECT_FALSE(_session->_final_handler_called);
}
...
TEST_F(AuthMiddlewareTest, CustomUserContextKey) {
    ...
    auto custom_key_check_handler = [&](std::shared_ptr<qb::http::Context<MockAuthSession>> ctx) {
...
TEST_F(AuthMiddlewareTest, CustomAuthScheme) {
    ...
    req_custom_scheme.set_header(custom_scheme_options.get_auth_header_name(), custom_scheme + " " + token);
...
TEST_F(AuthMiddlewareTest, ClockSkewTolerance) {
    ...
    for (auto &[k, v] : payload_exp_within_tolerance.items()) {
        map_exp_within_tolerance[k] = v.is_string() ? v.get<std::string>() : v.dump();
    }
    ...
    EXPECT_EQ(_session->_response.status(), qb::http::status::OK) << "Token expired 10s ago, but should be OK due to 20s tolerance.";
    EXPECT_TRUE(_session->_final_handler_called);
    ASSERT_TRUE(_session->_user_in_context.has_value());
    ...
    for (auto &[k, v] : payload_exp_beyond_tolerance.items()) {
        map_exp_beyond_tolerance[k] = v.is_string() ? v.get<std::string>() : v.dump();
    }
    ...
    for (auto &[k, v] : payload_nbf_within_tolerance.items()) {
        map_nbf_within_tolerance[k] = v.is_string() ? v.get<std::string>() : v.dump();
    }
    ...
    EXPECT_EQ(_session->_response.status(), qb::http::status::OK) << "Token NBF in 10s, but should be OK due to 20s tolerance.";
    EXPECT_TRUE(_session->_final_handler_called);
    ASSERT_TRUE(_session->_user_in_context.has_value());
    EXPECT_EQ(_session->_user_in_context->id, test_user.id);
    ...
    for (auto &[k, v] : payload_nbf_beyond_tolerance.items()) {
        map_nbf_beyond_tolerance[k] = v.is_string() ? v.get<std::string>() : v.dump();
    }
    ...
    EXPECT_EQ(_session->_response.status(), qb::http::status::UNAUTHORIZED) << "Token NBF in 30s, should be UNAUTHORIZED with 20s tolerance.";
    EXPECT_FALSE(_session->_final_handler_called);
}
...
TEST_F(AuthMiddlewareTest, IssuerAudienceFlexibility) {
    ...
    auto create_custom_token = [&](const qb::http::auth::User &user, const qb::http::auth::Options &creation_opts,
    ...
    if (!token_correct_iss_opt) {
        FAIL() << "S1.1 Token creation failed (correct issuer).";
        return;
    }
    ...
    if (!token_incorrect_iss_opt) {
        FAIL() << "S1.2 Token creation failed (incorrect issuer).";
        return;
    }
    ...
    if (!token_no_iss_opt) {
        FAIL() << "S1.3 Token creation failed (no issuer).";
        return;
    }
    ...
    configure_router_with_auth_mw(_auth_mw);
    std::optional<std::string> token_with_iss_verify_off_opt =
        create_custom_token(test_user, issuer_no_verify_opts, "any_app_iss", std::nullopt);
    if (!token_with_iss_verify_off_opt) {
        FAIL() << "S1.4 Token creation failed (issuer present, verify off).";
        return;
    }
    ...
    if (!token_correct_aud_opt) {
        FAIL() << "S2.1 Token creation failed (correct audience).";
        return;
    }
    ...
    if (!token_incorrect_aud_opt) {
        FAIL() << "S2.2 Token creation failed (incorrect audience).";
        return;
    }
    ...
    if (!token_no_aud_opt) {
        FAIL() << "S2.3 Token creation failed (no audience).";
        return;
    }
    ...
    configure_router_with_auth_mw(_auth_mw);
    std::optional<std::string> token_with_aud_verify_off_opt =
        create_custom_token(test_user, audience_no_verify_opts, std::nullopt, "any_client_aud");
    if (!token_with_aud_verify_off_opt) {
        FAIL() << "S2.4 Token creation failed (audience present, verify off).";
        return;
    }
    std::string token_with_aud_verify_off = *token_with_aud_verify_off_opt;
    auto        req2_4                    = create_request();
    req2_4.set_header(audience_no_verify_opts.get_auth_header_name(),
                      audience_no_verify_opts.get_auth_scheme() + " " + token_with_aud_verify_off);
...
TEST_F(AuthMiddlewareTest, OptionalAuthRejectsProvidedInvalidToken) {
    ...
    req_expired.set_header(_auth_options.get_auth_header_name(), _auth_options.get_auth_scheme() + " " + expired_token);
    ...
    req_wrong_sig.set_header(_auth_options.get_auth_header_name(), _auth_options.get_auth_scheme() + " " + token_wrong_sig);
...
TEST_F(AuthMiddlewareTest, TokenAndSchemeWhitespaceTolerance) {
    ...
    EXPECT_EQ(_session->_response.status(), qb::http::status::OK) << "Extra spaces between scheme and token should be accepted.";
    EXPECT_TRUE(_session->_final_handler_called);
    ASSERT_TRUE(_session->_user_in_context.has_value());
    EXPECT_EQ(_session->_user_in_context->id, test_user.id);
    ...
    EXPECT_EQ(_session->_response.status(), qb::http::status::OK) << "Trailing spaces after token should ideally be accepted or trimmed.";
    EXPECT_TRUE(_session->_final_handler_called);
    ASSERT_TRUE(_session->_user_in_context.has_value());
    ...
    EXPECT_EQ(_session->_response.status(), qb::http::status::UNAUTHORIZED) << "No space between scheme and token should be rejected.";
    EXPECT_FALSE(_session->_final_handler_called);
}
