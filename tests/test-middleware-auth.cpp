...
protected:
    ...
    std::string generate_test_token(const qb::http::auth::User &user, const std::string &secret_override = "") {
        qb::http::auth::Manager temp_manager(secret_override.empty()
                                                 ? _auth_options
                                                 : qb::http::auth::Options().secret_key(secret_override));
...
TEST_F(AuthMiddlewareTest, InvalidToken) {
    ...
    EXPECT_NE(_session->_response.body().template as<std::string>().find("Invalid or expired token"),
              std::string::npos);
    EXPECT_FALSE(_session->_final_handler_called);
}
...
TEST_F(AuthMiddlewareTest, CustomAuthScheme) {
    ...
    EXPECT_EQ(_session->_response.status(), qb::http::status::UNAUTHORIZED);
    EXPECT_NE(_session->_response.body().template as<std::string>().find("Invalid authentication format"),
              std::string::npos);
    EXPECT_FALSE(_session->_final_handler_called);
...
TEST_F(AuthMiddlewareTest, IssuerAudienceFlexibility) {
    ...
    configure_router_with_auth_mw(_auth_mw);
    std::optional<std::string> token_correct_iss_opt = create_custom_token(
        test_user, issuer_verify_opts, "my_app", std::nullopt);
    ...
    req1_1.set_header(issuer_verify_opts.get_auth_header_name(),
                      issuer_verify_opts.get_auth_scheme() + " " + token_correct_iss);
    ...
    _session->reset();
    std::optional<std::string> token_incorrect_iss_opt = create_custom_token(
        test_user, issuer_verify_opts, "other_app", std::nullopt);
    ...
    req1_2.set_header(issuer_verify_opts.get_auth_header_name(),
                      issuer_verify_opts.get_auth_scheme() + " " + token_incorrect_iss);
    ...
    _session->reset();
    std::optional<std::string> token_no_iss_opt = create_custom_token(test_user, issuer_verify_opts, std::nullopt,
                                                                      std::nullopt);
    if (!token_no_iss_opt) {
    ...
    req1_3.set_header(issuer_verify_opts.get_auth_header_name(),
                      issuer_verify_opts.get_auth_scheme() + " " + token_no_iss);
    ...
    configure_router_with_auth_mw(_auth_mw);
    std::optional<std::string> token_with_iss_verify_off_opt = create_custom_token(
        test_user, issuer_no_verify_opts, "any_app_iss", std::nullopt);
    ...
    req1_4.set_header(issuer_no_verify_opts.get_auth_header_name(),
                      issuer_no_verify_opts.get_auth_scheme() + " " + token_with_iss_verify_off);
    ...
    configure_router_with_auth_mw(_auth_mw);
    std::optional<std::string> token_correct_aud_opt = create_custom_token(
        test_user, audience_verify_opts, std::nullopt, "my_client");
    ...
    req2_1.set_header(audience_verify_opts.get_auth_header_name(),
                      audience_verify_opts.get_auth_scheme() + " " + token_correct_aud);
    ...
    _session->reset();
    std::optional<std::string> token_incorrect_aud_opt = create_custom_token(
        test_user, audience_verify_opts, std::nullopt, "other_client");
    ...
    req2_2.set_header(audience_verify_opts.get_auth_header_name(),
                      audience_verify_opts.get_auth_scheme() + " " + token_incorrect_aud);
    ...
    _session->reset();
    std::optional<std::string> token_no_aud_opt = create_custom_token(test_user, audience_verify_opts, std::nullopt,
                                                                      std::nullopt);
    if (!token_no_aud_opt) {
    ...
    req2_3.set_header(audience_verify_opts.get_auth_header_name(),
                      audience_verify_opts.get_auth_scheme() + " " + token_no_aud);
    ...
    configure_router_with_auth_mw(_auth_mw);
    std::optional<std::string> token_with_aud_verify_off_opt = create_custom_token(
        test_user, audience_no_verify_opts, std::nullopt, "any_client_aud");
...
TEST_F(AuthMiddlewareTest, OptionalAuthWithInvalidToken) {
    ...
    req_malformed.set_header(_auth_options.get_auth_header_name(),
