...
TEST_F(CorsAdvancedTest, PermissiveConfig) {
    ...
    // Test preflight
    auto preflight = createPreflightRequest("/api/users", "https://localhost:3000", "DELETE", 
...
TEST_F(CorsAdvancedTest, SecureConfig) {
    ...
    router->enable_cors(CorsOptions::secure({"https://app.example.com", "https://api.example.com"}));
    ...
    // Test preflight
    auto preflight = createPreflightRequest("/api/users", "https://api.example.com", "POST", 
...
TEST_F(CorsAdvancedTest, WildcardOriginWithCredentials) {
    // Enable CORS with wildcard origin and credentials (should result in specific origin being echoed)
    router->enable_cors(CorsOptions()
        .origins({"*"})
        .credentials(CorsOptions::AllowCredentials::Yes)
    );
...
TEST_F(CorsAdvancedTest, DynamicSubdomainsTest) {
    ...
    router->enable_cors_with_patterns({
        R"(^https:\/\/([a-zA-Z0-9-]+)\.tenant\.example\.com$)"  // tenant-specific subdomains
    });
    
    // Test with different tenant subdomains
    auto req1 = createRequest(HTTP_GET, "/api/users", "https://tenant1.tenant.example.com");
    router->route(session, req1);
    EXPECT_EQ(session.header("Access-Control-Allow-Origin"), "https://tenant1.tenant.example.com");
    
    // Reset session
    session.reset();
    
    // Test with another tenant
    auto req2 = createRequest(HTTP_GET, "/api/users", "https://large-corp-15.tenant.example.com");
    router->route(session, req2);
    EXPECT_EQ(session.header("Access-Control-Allow-Origin"), "https://large-corp-15.tenant.example.com");
    
    // Reset session
    session.reset();
    
    // Test with invalid subdomain format
    auto req3 = createRequest(HTTP_GET, "/api/users", "https://invalid@tenant.example.com");
    router->route(session, req3);
    EXPECT_EQ(session.header("Access-Control-Allow-Origin"), "");
    ...
    auto req4 = createRequest(HTTP_GET, "/api/users", "https://tenant1.tenant.example.org");
    router->route(session, req4);
    EXPECT_EQ(session.header("Access-Control-Allow-Origin"), "");
}
...
TEST_F(CorsAdvancedTest, MaxAgePreflightCacheTest) {
    ...
    for (int age : max_ages) {
        ...
        auto preflight = createPreflightRequest("/api/users", "https://app.example.com", "PUT", 
                                           {"Content-Type", "Authorization"});
        router->route(session, preflight);
...
TEST_F(CorsAdvancedTest, MultipleMatchingStrategiesTest) {
    ...
    std::function<bool(const std::string&)> combined_matcher = [](const std::string& origin) {
        // Direct matches
        ...
        std::vector<std::regex> patterns = {
            ...
            std::regex(R"(^https:\/\/dashboard\-([a-zA-Z0-9]+)\.example\.(com|org)$)")
        };
    ...
    router->enable_cors(CorsOptions()
        .origin_matcher(combined_matcher)
        .all_methods()
        .common_headers()
    );
    ...
    auto req3 = createRequest(HTTP_GET, "/api/users", "https://dashboard-user1.example.org");
    router->route(session, req3);
    EXPECT_EQ(session.header("Access-Control-Allow-Origin"), "https://dashboard-user1.example.org");
