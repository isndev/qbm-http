 ...
 * (auto-detected via `CoroRouteHandler` / `CoroMiddlewareHandler` concepts,
 * declared in `qbm/http/routing/coro_task.h`). The tests run against a real
 * HTTP/1.1 server so the full routing chain is observed:
 *
 *   - A `task<void>(ctx)` lambda passed directly to a verb method executes on
 *     the server's thread-local coroutine scheduler and the framework
 *     auto-completes the request.
 *   - A `task<void>(ctx)` lambda passed to `use()` observes / mutates the
 *     context and continues the chain without an explicit `next()` call.
