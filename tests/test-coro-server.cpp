...
public:
    CoroServer() {
        ...
        router().use(qb::http::coro_middleware<Session>([](auto ctx) -> qb::io::async::task<void> {
        ...
        router().get("/coro/hello", qb::http::coro_handler<Session>([](auto ctx) -> qb::io::async::task<void> {
                         co_await qb::io::async::sleep(1ms);
                         ctx->response().status() = qb::http::status::OK;
                         ctx->response().body()   = std::string{"hi:"} + ctx->request().header("X-Seen-By-Coro-MW");
                         co_return;
                     }));

        router().get("/coro/explicit", qb::http::coro_handler<Session>([](auto ctx) -> qb::io::async::task<void> {
        ...
        router().get("/coro/boom", qb::http::coro_handler<Session>([](auto /*ctx*/) -> qb::io::async::task<void> {
                         co_await qb::io::async::sleep(1ms);
        ...
        group->use(qb::http::coro_middleware<Session>([](auto ctx) -> qb::io::async::task<void> {
            co_await qb::io::async::sleep(1ms);
        ...
        group->get("/chain", qb::http::coro_handler<Session>([](auto ctx) -> qb::io::async::task<void> {
                       ctx->response().status() = qb::http::status::OK;
        ...
        group->get("/gate", qb::http::coro_handler<Session>([](auto ctx) -> qb::io::async::task<void> {
                       ctx->response().status() = qb::http::status::OK;
        ...
        group->get("/manual-continue", qb::http::coro_handler<Session>([](auto ctx) -> qb::io::async::task<void> {
                       ctx->response().status() = qb::http::status::OK;
