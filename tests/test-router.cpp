...
TEST_F(RouterTest, PostRouteSimple) {
    router.post("/create", [](auto ctx) {
        ctx->response().status_code = HTTP_STATUS_CREATED; // 201
    ...
    ASSERT_EQ(mock_session->_response.status_code, HTTP_STATUS_CREATED);
...
TEST_F(RouterTest, AddAndMatchSimpleDeleteRoute) {
    router.del("/resource/456", [](auto ctx) { // Assuming .del() or .delete_()
        ctx->response().status_code = HTTP_STATUS_NO_CONTENT; // 204 No Content is common
        ctx->complete();
    });
    ...
    ASSERT_EQ(mock_session->_response.status_code, HTTP_STATUS_NO_CONTENT);
...
TEST_F(RouterTest, MiddlewareShortCircuitingResponse) {
    ...
    ASSERT_EQ(mock_session->_response.status_code, HTTP_STATUS_UNAUTHORIZED);
