...
namespace static_table_detail {
     ...
     */
    constexpr std::uint32_t name_hash(std::string_view s) noexcept {
     ...
     */
    constexpr std::array<NameIndexSlot, kNameIndexCapacity> build_name_index() {
        ...
        for (std::size_t i = 0; i < STATIC_TABLE.size(); ++i) {
            ...
            for (std::size_t probe = 0; probe < kNameIndexCapacity; ++probe) {
                ...
                if (e.key_hash == h &&
                    STATIC_TABLE[e.table_index - 1].first == name) {
    ...
    kStaticNameIndex = build_name_index();
} // namespace static_table_detail
...
public:
    ...
    [[nodiscard]] std::size_t byte_size() const noexcept { return _bytes; }
    [[nodiscard]] std::size_t max_byte_size() const noexcept { return _max_bytes; }
    ...
    [[nodiscard]] const DynamicTableEntry &operator[](std::size_t i) const noexcept {
        return _storage[slot_of(i)];
    }

    /// The oldest entry; undefined behaviour when the table is empty.
    [[nodiscard]] const DynamicTableEntry &back() const noexcept {
     ...
     */
    void clear() noexcept { clear_storage(); }
...
private:
    ...
    [[nodiscard]] std::size_t slot_of(std::size_t logical) const noexcept {
        return (_head + logical) & _mask;
    }

    void evict_oldest_impl() noexcept {
...
struct HeaderField {
     ...
     */
    [[nodiscard]] bool is_sensitive_by_default() const noexcept {
...
namespace static_table {
    ...
    find_exact_match(std::string_view name, std::string_view value) noexcept {
        ...
        for (std::size_t i = *first - 1; i < STATIC_TABLE.size(); ++i) {
            ...
            if (entry.first != name) break; // Left the contiguous same-name run.
     ...
     */
    [[nodiscard]] inline bool is_valid_index(std::size_t index) noexcept {
...
public:
        ...
        while (current_pos < end_pos) {
            ...
            if (first_byte >> 7 == 1) { // Indexed Header Field (1xxxxxxx)
                ...
                _stats.dynamic_table_evictions +=
                    _dynamic_table.set_max_byte_size(static_cast<std::size_t>(new_max_size));
            } else {
    ...
    void set_max_header_list_size(uint32_t max_list_size) {
    ...
    [[nodiscard]] const HpackStats& get_stats() const {
        return _stats;
    }

    [[nodiscard]] std::size_t get_dynamic_table_size() const {
        return _dynamic_table.byte_size();
    }

    [[nodiscard]] std::size_t get_dynamic_table_entry_count() const {
...
private:
     ...
     */
    void encode_string_literal(std::vector<uint8_t>& buffer, std::string_view str,
                               bool huffman_allowed = true) const {
...
public:
    ...
    void set_peer_max_dynamic_table_size(uint32_t max_size) {
        ...
        if (_dynamic_table.max_byte_size() > _peer_max_dynamic_table_octets) {
            _stats.dynamic_table_evictions +=
                _dynamic_table.set_max_byte_size(_peer_max_dynamic_table_octets);
        }
    }

    void set_max_capacity(uint32_t max_capacity) {
        _stats.dynamic_table_evictions +=
            _dynamic_table.set_max_byte_size(max_capacity);
    }
