...
public:
     ...
     */
    std::size_t
    set_max_byte_size(std::size_t new_max) {
        _max_bytes          = new_max;
        std::size_t evicted = 0;
...
private:
     ...
     */
    [[nodiscard]] static std::pair<uint64_t, int>
    decode_integer(const uint8_t *&current_pos, const uint8_t *end_pos, uint8_t N) {
     ...
     */
    [[nodiscard]] static std::pair<std::optional<std::string>, int>
    decode_string_literal(const uint8_t *&current_pos, const uint8_t *end_pos, bool &out_is_possibly_incomplete, HpackStats &stats) {
        if (current_pos >= end_pos) {
     ...
     */
    [[nodiscard]] bool
    get_dynamic_table_entry(uint64_t index, std::string &name, std::string &value) const {
     ...
     */
    [[nodiscard]] bool
    get_name_from_index(uint64_t index, std::string &name) const {
        if (index <= STATIC_TABLE.size()) {
...
public:
    ...
    bool
    decode(const std::vector<uint8_t> &encoded_block, std::vector<HeaderField> &out_headers, bool &out_is_possibly_incomplete) {
...
private:
     ...
     */
    static void
    encode_integer(std::vector<uint8_t> &buffer, uint8_t prefix_bits, uint8_t N, uint64_t value) {
     ...
     */
    void
    encode_string_literal(std::vector<uint8_t> &buffer, std::string_view str, bool huffman_allowed = true) const {
...
public:
    ...
    bool
    encode(const std::vector<HeaderField> &headers_to_encode, std::vector<uint8_t> &out_buffer) {
