...
extract_uint16_be(const uint8_t *data) noexcept {
...
extract_uint32_be(const uint8_t *data) noexcept {
 ...
 */
[[nodiscard]] inline uint32_t
extract_uint31_be(const uint8_t *data) noexcept {
 ...
 */
inline void
encode_uint16_be(uint16_t value, uint8_t *data) noexcept {
 ...
 */
inline void
encode_uint32_be(uint32_t value, uint8_t *data) noexcept {
...
public:
    ...
    struct ValidationResult {
        ...
        static ValidationResult
        invalid(ErrorCode code, std::string msg) {
     ...
     */
    static ValidationResult
    validate_setting(Http2SettingIdentifier id, uint32_t value, bool /*is_from_client*/) {
        switch (id) {
     ...
     */
    static qb::unordered_map<Http2SettingIdentifier, uint32_t>
    get_default_settings(bool is_server) {
     ...
     */
    static uint32_t
    calculate_safe_max_frame_size(const qb::unordered_map<Http2SettingIdentifier, uint32_t> &settings) {
...
public:
     ...
     */
    static bool
    is_forbidden_header(std::string_view name) {
     ...
     */
    static bool
    is_forbidden_response_header(std::string_view name) {
     ...
     */
    static bool
    is_forbidden_trailer_header(std::string_view name) {
    ...
    is_valid_request_te_value(std::string_view value) noexcept {
     ...
     */
    static bool
    is_pseudo_header(std::string_view name) {
    ...
    is_valid_header_field(std::string_view name, std::string_view value) noexcept {
        ...
        for (unsigned char c : name) {
            ...
            const bool is_valid_char = (c >= 'a' && c <= 'z') || (c >= '0' && c <= '9') || c == '!' || c == '#' || c == '$' || c == '%'
                                       || c == '&' || c == '\'' || c == '*' || c == '+' || c == '-' || c == '.' || c == '^' || c == '_'
                                       || c == '`' || c == '|' || c == '~';
            if (!is_valid_char) {
                return false;
     ...
     */
    [[nodiscard]] static bool
    validate_pseudo_header_order(const std::vector<qb::protocol::hpack::HeaderField> &headers) noexcept {
     ...
     */
    static SettingsHelper::ValidationResult
    validate_request_pseudo_headers(const std::vector<qb::protocol::hpack::HeaderField> &headers) {
     ...
     */
    static SettingsHelper::ValidationResult
    validate_response_pseudo_headers(const std::vector<qb::protocol::hpack::HeaderField> &headers) {
...
public:
     ...
     */
    static bool
    should_escalate_to_connection(ErrorCode error_code, uint32_t stream_id) {
     ...
     */
    static std::string
    format_error_message(ErrorCode error_code, const std::string &context, uint32_t stream_id = 0) {
        std::string message;
    ...
    get_error_name(ErrorCode error_code) {
        switch (error_code) {
...
public:
     ...
     */
    static bool
    is_valid_client_stream_id(uint32_t stream_id) {
     ...
     */
    static bool
    is_valid_server_stream_id(uint32_t stream_id) {
     ...
     */
    static bool
    is_connection_stream_id(uint32_t stream_id) {
        return stream_id == 0;
    }
     ...
     */
    static SettingsHelper::ValidationResult
    validate_stream_id_for_frame(FrameType frame_type, uint32_t stream_id, bool is_server) {
        switch (frame_type) {
...
private:
     ...
     */
    static std::string_view
    get_frame_type_name(FrameType frame_type) {
        switch (frame_type) {
            case FrameType::DATA:
                return "DATA";
            case FrameType::HEADERS:
...
public:
    ...
    explicit Http2Protocol(IO_Handler &io_handler_ref, uint32_t our_max_frame_size_setting)
        : Base(io_handler_ref)
        , _our_max_frame_size(our_max_frame_size_setting) {
     ...
     */
    [[nodiscard]] const FrameHeader &
    get_current_frame_header_for_error() const {
     ...
     */
    void
    onMessage(std::size_t received_size) noexcept override {
            ...
            switch (_current_state) {
                case ParserState::EXPECTING_PREFACE: {
                    ...
                    if (bytes_to_copy > 0) {
                        ...
                        _preface_buffer.insert(_preface_buffer.end(), in_buffer.cbegin(), in_buffer.cbegin() + bytes_to_copy);
                    }

                    if (_preface_buffer.size() == HTTP2_CONNECTION_PREFACE.size()) {
                        if (std::memcmp(_preface_buffer.data(), HTTP2_CONNECTION_PREFACE.data(), HTTP2_CONNECTION_PREFACE.size()) == 0) {
                ...
                case ParserState::EXPECTING_FRAME_HEADER: {
                    ...
                    if (_continuation_required) {
                        if (_current_frame_header.get_type() != FrameType::CONTINUATION
                            ...
                            static_cast<SideProtocol *>(this)->handle_framer_detected_error(
                                ErrorCode::PROTOCOL_ERROR, "Expected CONTINUATION frame for stream " + std::to_string(_continuation_stream_id),
                    ...
                    if (_current_frame_header.get_type() == FrameType::SETTINGS && _current_frame_header.get_stream_id() != 0) {
                        this->not_ok(ErrorCode::PROTOCOL_ERROR);
                        return;
                    }

                    // Validate PING/GOAWAY frames on stream 0
                    if ((_current_frame_header.get_type() == FrameType::PING || _current_frame_header.get_type() == FrameType::GOAWAY)
                ...
                case ParserState::EXPECTING_FRAME_PAYLOAD: {
                    if (received_size != _expected_payload_bytes) {
                        this->not_ok(ErrorCode::FRAME_SIZE_ERROR);
                        static_cast<SideProtocol *>(this)->handle_framer_detected_error(ErrorCode::FRAME_SIZE_ERROR, "Payload size incorrect",
...
protected:
    [[nodiscard]] bool
    send_headers_with_continuation(uint32_t stream_id, uint8_t first_frame_flags, std::vector<uint8_t> &&encoded_header_block) noexcept {
        const auto  max_fragment_size = static_cast<std::size_t>(std::max<uint32_t>(1, this->get_peer_max_frame_size()));
        std::size_t offset            = 0;
     ...
     */
    void
    not_ok(ErrorCode reason) noexcept {
     ...
     */
    [[nodiscard]] bool
    report_frame_error(ErrorCode error_code, const std::string &message, uint32_t stream_id = 0) noexcept {
     ...
     */
    [[nodiscard]] bool
    validate_payload_size(std::size_t expected_size, std::size_t actual_size, const std::string &frame_type_name, uint32_t stream_id) noexcept {
        if (expected_size != actual_size) {
            return report_frame_error(ErrorCode::FRAME_SIZE_ERROR, frame_type_name + " frame payload incorrect size.", stream_id);
        }
        return true;
    }
     ...
     */
    [[nodiscard]] bool
    validate_min_payload_size(std::size_t min_size, std::size_t actual_size, const std::string &frame_type_name, uint32_t stream_id) noexcept {
        if (actual_size < min_size) {
            return report_frame_error(ErrorCode::FRAME_SIZE_ERROR, frame_type_name + " frame payload too short.", stream_id);
        }
        return true;
    }
     ...
     */
    [[nodiscard]] std::pair<uint8_t, bool>
    validate_padded_frame(const uint8_t *&p_data, std::size_t &p_len, const std::string &frame_type_name, uint32_t stream_id) noexcept {
        if (p_len == 0) {
            (void) report_frame_error(ErrorCode::FRAME_SIZE_ERROR, "Padded " + frame_type_name + " frame too short for Pad Length.", stream_id);
            return {0, false};
        }
        ...
        if (pad_length > p_len) {
            (void) report_frame_error(ErrorCode::PROTOCOL_ERROR, "Pad Length in " + frame_type_name + " frame exceeds payload size.",
                                      stream_id);
            return {0, false};
        }
    ...
    [[nodiscard]] bool
    send_frame_internal(const FrameHeader &header, const FramePayloadType &payload, std::size_t payload_size_for_header) noexcept {
     ...
     */
    void
    on_connection_error(ErrorCode error_code, const std::string &debug_message) {
     ...
     */
    void
    on_stream_error(uint32_t stream_id, ErrorCode error_code, const std::string &debug_message) {
     ...
     */
    [[nodiscard]] uint32_t
    get_our_max_frame_size() const noexcept {
     ...
     */
    [[nodiscard]] uint32_t
    get_last_peer_initiated_stream_id_processed_in_goaway() const noexcept {
     ...
     */
    void
    set_last_peer_initiated_stream_id_processed_in_goaway(uint32_t stream_id) noexcept {
     ...
     */
    [[nodiscard]] uint32_t
    get_peer_max_frame_size() const noexcept {
     ...
     */
    void
    set_peer_max_frame_size(uint32_t size) noexcept {
        _peer_max_frame_size = std::max(qb::protocol::http2::DEFAULT_MAX_FRAME_SIZE, std::min(size, qb::protocol::http2::MAX_FRAME_SIZE_LIMIT));
    }
     ...
     */
    [[nodiscard]] bool
    are_all_relevant_streams_closed(uint32_t last_processed_peer_stream_id) const {
...
private:
     ...
     */
    void
    handle_zero_payload_frame_dispatch() noexcept {
        if (!this->ok())
            return;
        ...
        switch (_current_frame_header.get_type()) {
            ...
            case FrameType::PING:
                ...
                this->not_ok(ErrorCode::FRAME_SIZE_ERROR);
                static_cast<SideProtocol *>(this)->handle_framer_detected_error(ErrorCode::FRAME_SIZE_ERROR, "PING frame with zero payload.",
                                                                                _current_frame_header.get_stream_id());
                break;
            ...
            default:
                ...
                // QB_LOG_TRACE_PA(this->getName(), "Ignored unknown frame type " << static_cast<int>(_current_frame_header.get_type()) << "
                // with zero payload.");
                break;
        }
    }
     ...
     */
    [[nodiscard]] bool
    handle_payload_frame_dispatch(std::string_view payload_view) noexcept {
        if (!this->ok())
            return false;
     ...
     */
    [[nodiscard]] bool
    handle_data_frame_payload(const uint8_t *payload_data, std::size_t payload_size) noexcept {
     ...
     */
    [[nodiscard]] bool
    handle_headers_frame_payload(const uint8_t *payload_data, std::size_t payload_size) noexcept {
     ...
     */
    [[nodiscard]] bool
    handle_priority_frame_payload(const uint8_t *payload_data, std::size_t payload_size) noexcept {
     ...
     */
    [[nodiscard]] bool
    handle_rst_stream_frame_payload(const uint8_t *payload_data, std::size_t payload_size) noexcept {
     ...
     */
    [[nodiscard]] bool
    handle_settings_frame_payload(const uint8_t *payload_data, std::size_t payload_size) noexcept {
        if (_current_frame_header.flags & FLAG_ACK) {
            return report_frame_error(ErrorCode::FRAME_SIZE_ERROR, "SETTINGS ACK frame with payload.", _current_frame_header.get_stream_id());
        }

        if (payload_size % 6 != 0) {
            return report_frame_error(ErrorCode::FRAME_SIZE_ERROR, "SETTINGS frame payload size not a multiple of 6.",
                                      _current_frame_header.get_stream_id());
        }
     ...
     */
    [[nodiscard]] bool
    handle_push_promise_frame_payload(const uint8_t *payload_data, std::size_t payload_size) noexcept {
     ...
     */
    [[nodiscard]] bool
    handle_ping_frame_payload(const uint8_t *payload_data, std::size_t payload_size) noexcept {
     ...
     */
    [[nodiscard]] bool
    handle_goaway_frame_payload(const uint8_t *payload_data, std::size_t payload_size) noexcept {
     ...
     */
    [[nodiscard]] bool
    handle_window_update_frame_payload(const uint8_t *payload_data, std::size_t payload_size) noexcept {
     ...
     */
    [[nodiscard]] bool
    handle_continuation_frame_payload(const uint8_t *payload_data, std::size_t payload_size) noexcept {
