...
public:
     ...
     */
    void
    reset() noexcept { // noexcept if String clear/assign and CookieJar::clear are noexcept
        _status = Status::OK;
        _cookies.clear();
                ...
                std::remove_if(set_cookie_headers.begin(), set_cookie_headers.end(),
                               [&](const String& header_val) {
                                   ...
                                   if constexpr (std::is_convertible_v<const String&, std::string_view>) {
                                       ...
                                       return std::string(header_val.data(), header_val.length()).rfind(prefix_to_find, 0) == 0;
                                   }
                               }),
