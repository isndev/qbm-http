 ...
 * - If the predicate returns `false` and no `else_middleware` is provided, this middleware simply calls
 * `ctx->complete(AsyncTaskResult::CONTINUE)` to pass control to the next task in the main chain.
 *
...
public:
     ...
     */
    ConditionalMiddleware(Predicate predicate, ChildMiddlewarePtr if_middleware, ChildMiddlewarePtr else_middleware = nullptr,
...
conditional_middleware(typename ConditionalMiddleware<SessionType>::Predicate predicate,
                       std::shared_ptr<IMiddleware<SessionType>>              if_middleware,
                       std::shared_ptr<IMiddleware<SessionType>> else_middleware = nullptr, const std::string &name = "ConditionalMiddleware") {
    return std::make_shared<ConditionalMiddleware<SessionType>>(std::move(predicate), std::move(if_middleware), std::move(else_middleware),
                                                                name);
}
} // namespace qb::http
