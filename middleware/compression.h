...
public:
     ...
     * @return Reference to this `CompressionOptions` instance for chaining.
     */
    CompressionOptions& preferred_encodings(std::vector<std::string> encodings) { // Can allocate
    ...
    [[nodiscard]] const std::vector<std::string>& get_preferred_encodings() const noexcept { return _preferred_encodings; }
};
...
public:
    ...
    void process(ContextPtr ctx) override {
        ...
        if (_options.should_decompress_requests() && can_decompress_request(ctx->request())) {
            try {
                ...
                return;
            } catch (const std::exception& e) { // Catch other potential exceptions from Body::uncompress
     ...
     */
    void update_options(CompressionOptions opts) noexcept { // Pass by value for potential move
...
private:
    ...
    void decompress_request_body(RequestType& request) {
        ...
        } else { // Assume std::string_view or convertible
        ...
        if (request.has_header("Content-Length")) { // Update Content-Length if present
    ...
    void compress_response_body(Context<SessionType>& ctx_ref) { 
        ...
        if (response.body().size() < _options.get_min_size_to_compress() ||
            ...
            is_precompressed_content_type(std::string(response.content_type().type()))) { // Use parsed ContentType
            return;
        }
        ...
        if (compressed_size > 0 && compressed_size < response.body().raw().size()) { // Only set headers if compression was effective
    ...
    [[nodiscard]] bool is_precompressed_content_type(const std::string& content_type_header) const noexcept {
        static const std::vector<std::string_view> compressed_mime_types = { // Use string_view for efficiency
