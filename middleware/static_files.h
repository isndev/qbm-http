...
namespace internal {
    ...
    sanitize_and_resolve_path(const std::filesystem::path& base_path, std::string_view original_relative_path_sv) {
        ...
        if (resolved_str.rfind(base_str, 0) == 0) { // Check if resolved_str starts with base_str
    ...
    inline std::string get_mime_type_for_file(const std::filesystem::path& file_path, const StaticFilesOptions& opts) {
    ...
    parse_byte_range(std::string_view range_header_value, long long total_file_size) {
        if (range_header_value.rfind("bytes=", 0) != 0) { // Must start with "bytes="
            return std::nullopt;
        }
        ...
        if (!start_str.empty()) {
            try {
                ...
                if (parsed_chars_start != start_str.length()) return std::nullopt; // Ensure all of start_str was number
        ...
        if (start != -1 && end != -1) { // bytes=start-end
    ...
    inline std::string generate_directory_listing_html(
        ...
        if ((dir_display_name_str.empty() || dir_display_name_str == ".") && directory_path.has_parent_path()) {
            ...
            if (directory_path.lexically_normal() == opts.root_directory.lexically_normal() || 
                (parent_of_current_dir.empty() || parent_of_current_dir == directory_path) ) { // at root
        ...
        if (dir_display_name_str.empty() && request_uri_path.length() > 1) { // Fallback for root if path was "/"
        ...
        if (ec) { // Error during iteration itself
...
public:
    ...
    explicit StaticFilesMiddleware(StaticFilesOptions options, std::string name = "StaticFilesMiddleware")
        ...
        if (!std::filesystem::exists(_options.root_directory, ec) || !std::filesystem::is_directory(_options.root_directory, ec)) {
            if (ec) {
                 throw std::runtime_error("Error checking StaticFilesOptions::root_directory: " + _options.root_directory.string() + " - " + ec.message());
            } else {
                 throw std::runtime_error("StaticFilesOptions::root_directory does not exist or is not a directory: " + _options.root_directory.string());
            }
        }
        ...
        if (ec) {
            throw std::runtime_error("Failed to get canonical path for root_directory: " + _options.root_directory.string() + " - " + ec.message());
        }
    }
    ...
    void process(ContextPtr ctx) override {
        ...
        if (is_dir) { // Explicitly a directory, try to serve index or list
            ...
            if (is_dir) { // Still a directory (index not found/served or not enabled)
                if (_options.enable_directory_listing) {
                    ...
                    if (_options.set_cache_control_header) { // Apply cache control for directory listing too
                        ctx->response().set_header("Cache-Control", _options.cache_control_value); // Or a different policy for listings?
                    }
                    if (is_head_request) {
        ...
        if (_options.enable_etags || _options.enable_last_modified) {
            ...
            if (_options.enable_etags) {
                ...
                auto last_modified_epoch_sec = std::chrono::duration_cast<std::chrono::seconds>(last_modified_sys_tp_for_etag.time_since_epoch()).count();
                ...
                if (!if_none_match_sv.empty()) {
                    ...
                    if (if_none_match_sv.find(etag_value) != std::string_view::npos) { // Check if our ETag is in the list
            ...
            if (_options.enable_last_modified) {
                ...
                std::string last_modified_str = qb::http::date::to_string(last_modified_sys_tp); // Uses your existing date formatter
        ...
        if (should_send_416) { // This implies range requests were enabled and an issue occurred
        ...
        ctx->response().set_header("Content-Length", std::to_string(length_to_read)); // Length of actual data being sent
