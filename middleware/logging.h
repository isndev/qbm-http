...
class LoggingMiddleware final : public IMiddleware<SessionType> {
public:
    ...
    process(ContextPtr ctx) override {
        ...
        ctx->add_lifecycle_hook([log_function    = std::move(log_function), response_level,
            ...
            if (point == HookPoint::PRE_RESPONSE_SEND) {
                ...
                std::string message =
                    "Response: " + std::to_string(static_cast<int>(response.status())) + " " + std::to_string(response.status());
                try {
                    log_function(response_level, message);
                } catch (...) {
                    LOG_HTTP_WARN("LoggingMiddleware [" << middleware_name << "]: Exception in user log function suppressed");
                }
            }
        });
...
private:
     ...
     */
    void
    log_request(const Request &request) {
        if (!_log_function)
            return; // Should not happen due to constructor check
     ...
     */
    void
    log_response(const Response &response) {
        if (!_log_function)
            return;
     ...
     */
    [[nodiscard]] std::string
    format_request_info(const Request &request) const {
     ...
     */
    [[nodiscard]] std::string
    format_response_info(const Response &response) const {
...
logging_middleware(typename LoggingMiddleware<SessionType>::LogFunction log_fn, LogLevel request_level = LogLevel::Info,
                   LogLevel response_level = LogLevel::Debug, const std::string &name = "LoggingMiddleware") {
    return std::make_shared<LoggingMiddleware<SessionType>>(std::move(log_fn), request_level, response_level, name);
}
} // namespace qb::http
