...
class ErrorHandlingMiddleware final : public IMiddleware<SessionType> {
public:
    ...
    on_status(qb::http::status status_code, StatusHandler handler) {
     ...
     */
    ErrorHandlingMiddleware &
    on_status_range(qb::http::status min_status, qb::http::status max_status, StatusHandler handler) {
        // Map emplace can throw
        if (handler) {
            for (int code_val = static_cast<int>(min_status); code_val <= static_cast<int>(max_status); ++code_val) {
     ...
     */
    ErrorHandlingMiddleware &
    on_any_error(ErrorHandler handler) {
    ...
    process(ContextPtr ctx) override {
        ...
        if (!handled && _generic_handler) {
            std::string error_message_for_generic_handler = "Error encountered: status " + std::to_string(static_cast<int>(current_status));
            if (auto err_msg_from_ctx = ctx->template get<std::string>("__error_message"); err_msg_from_ctx && !err_msg_from_ctx->empty()) {
