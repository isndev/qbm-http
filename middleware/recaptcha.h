...
public:
     ...
     */
    RecaptchaOptions& secret_key(std::string key) { // Renamed from const std::string& to std::string for move
     ...
     * @return Reference to this `RecaptchaOptions` instance for chaining.
     */
    RecaptchaOptions& api_url(std::string url) { // Renamed from const std::string&
     ...
     * @return Reference to this `RecaptchaOptions` instance for chaining.
     */
    RecaptchaOptions& from_header(std::string header_name) { // Renamed from const std::string&
     ...
     * @return Reference to this `RecaptchaOptions` instance for chaining.
     */
    RecaptchaOptions& from_body(std::string field_name) { // Renamed from const std::string&
     ...
     * @return Reference to this `RecaptchaOptions` instance for chaining.
     */
    RecaptchaOptions& from_query(std::string param_name) { // Renamed from const std::string&
...
struct RecaptchaResult { 
    ...
    std::string error_codes;     ///< Comma-separated list of error codes if success is false (e.g., "missing-input-secret", "invalid-input-response").
    std::chrono::system_clock::time_point challenge_ts; ///< Timestamp of the challenge load (when the reCAPTCHA was solved). Populated if available from Google's response.
};
...
public:
    ...
    RecaptchaMiddleware(
        ...
        _name(std::move(name)) {
         if (_options->get_secret_key().empty()) { // v3 factory also checks, but good to be explicit.
        ...
        qb::http::request(std::move(api_req), 
            ...
            if (!verification_result.success || verification_result.score < _options->get_min_score()) {
                ...
                if (!verification_result.success && !verification_result.error_codes.empty()) {
                    ...
                    error_detail += " Score (" + std::to_string(verification_result.score) + ") is below threshold (" + std::to_string(_options->get_min_score()) + ").";
                }
     ...
     */
    RecaptchaMiddleware& update_options(RecaptchaOptions opts) { // Pass by value for move
...
private:
    ...
    [[nodiscard]] std::optional<std::string> extract_token_from_request(const qb::http::Request& request) const {
        ...
        switch (_options->get_token_location()) {
            case RecaptchaOptions::TokenLocation::Header:{
                ...
                const auto& header_val_obj = request.header(field_name); // Default value of header() is empty string_type
                ...
                } else { // Fallback, assumes .data() and .length()
                    header_val_str.assign(header_val_obj.data(), header_val_obj.length());
                }
                return header_val_str.empty() ? std::nullopt : std::optional<std::string>(std::move(header_val_str));
            }
    ...
    [[nodiscard]] RecaptchaResult parse_google_recaptcha_response(const qb::http::Response& google_response) const {
        ...
        try {
            ...
            if (json_body.contains("score") && json_body["score"].is_number()) { // Score is for v3
                result.score = json_body["score"].get<float>();
            }
            if (json_body.contains("action") && json_body["action"].is_string()) { // Action is for v3
     ...
     */
    void set_error_response(ContextPtr ctx, qb::http::status status, const std::string& error_message, const std::string& details = "") const {
