...
public:
    ...
    void process(ContextPtr ctx) override {
        ...
        if (ctx->has("jwt_payload")) { // "jwt_payload" is a conventional key
        ...
        if (!user_opt) {
            ...
            if (token.empty()) { // Header present but format incorrect or no token after scheme
                if (_require_auth) {
        ...
        if (!user_opt) { // Still no user after trying context and header+token
            if (_require_auth) {
     ...
     * @return Reference to this `AuthMiddleware` instance for chaining.
     */
    AuthMiddleware& with_roles(std::vector<std::string> roles, bool require_all = false) { // Can alloc
...
create_jwt_auth_middleware(
    ...
    if (algorithm_str.rfind("HS", 0) == 0) { // Starts with HS -> HMAC
        options.secret_key(secret);
    } else { // Assume it's a public key for RSA/EC/EdDSA
