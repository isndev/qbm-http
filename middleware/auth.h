...
public:
     ...
     */
    explicit AuthMiddleware(const auth::Options &options, std::string name = "AuthMiddleware")
    ...
    process(ContextPtr ctx) override {
        ...
        if (!user_opt) {
            ...
            if (auth_header_str.empty()) {
                if (_require_auth) {
                    handle_auth_error(ctx, qb::http::status::UNAUTHORIZED, "Authentication required: Missing token or authorization header.");
                    return;
                }
            ...
            if (token.empty()) {
                ...
                handle_auth_error(ctx, qb::http::status::UNAUTHORIZED, "Invalid authentication format in header.");
                return;
            }
        ...
        if (!user_opt) {
            ...
            handle_auth_error(ctx, qb::http::status::UNAUTHORIZED, "Invalid or expired token; user authentication failed.");
            return;
        }
        ...
        if (!_required_roles.empty()) {
            bool authorized_by_role = _require_all_roles ? user_opt->has_all_roles(_required_roles) : user_opt->has_any_role(_required_roles);

            if (!authorized_by_role) {
                handle_auth_error(ctx, qb::http::status::FORBIDDEN, "Insufficient permissions based on user roles.");
                return;
            }
        }
     ...
     */
    AuthMiddleware &
    with_user_context_key(std::string key) noexcept {
     ...
     */
    AuthMiddleware &
    with_auth_required(bool required) noexcept {
     ...
     */
    AuthMiddleware &
    with_roles(std::vector<std::string> roles, bool require_all = false) {
        // Can alloc
     ...
     */
    AuthMiddleware &
    with_options(const auth::Options &options) noexcept {
    ...
    [[nodiscard]] auth::Manager &
    auth_manager() noexcept {
        return _auth_manager;
    }
    /** @brief Gets a constant reference to the internal `auth::Manager`. */
    [[nodiscard]] const auth::Manager &
    auth_manager() const noexcept {
        return _auth_manager;
    }
     ...
     */
    [[nodiscard]] std::string
    generate_token(const auth::User &user) const {
     ...
     */
    [[nodiscard]] std::optional<auth::User>
...
private:
     ...
     */
    void
    handle_auth_error(ContextPtr ctx, qb::http::status status_code, const std::string &message) {
     ...
     */
    [[nodiscard]] std::optional<auth::User>
    user_from_jwt_payload(const qb::json &jwt_payload) const {
        if (!jwt_payload.is_object())
            return std::nullopt;
        ...
        if (jwt_payload.contains("username") && jwt_payload["username"].is_string()) {
            ...
            if (!user.username.empty() && !id_found)
                id_found = true;
        }
