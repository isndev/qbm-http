...
namespace qb::http { // Middleware typically resides in qb::http namespace
...
public:
    ...
    void process(ContextPtr ctx) override {
        ...
        if (is_valid) {
            ...
            ctx->response().status() = qb::http::status::BAD_REQUEST; // Or qb::http::status::UNPROCESSABLE_ENTITY (422)
