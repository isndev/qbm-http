...
namespace qb::http {
    ...
    public:
        ...
        static std::shared_ptr<JwtMiddleware<SessionType> > create(const std::string &secret,
    ...
    private:
        ...
        std::optional<qb::json> verify_token(const std::string &token, JwtErrorInfo &error_info) const {
            ...
            if (!alg_opt) {
                error_info = {
                    JwtError::ALGORITHM_MISMATCH,
                    "JWT algorithm '" + _options.algorithm + "' is not supported or recognized by the library."
                };
                return std::nullopt;
            }
            ...
            if (result.is_valid()) {
                ...
                switch (result.error) {
                    case qb::jwt::ValidationError::INVALID_FORMAT: error_info = {
                                                                       JwtError::INVALID_TOKEN, "Invalid token format."
                                                                   };
                        break;
                    case qb::jwt::ValidationError::INVALID_SIGNATURE: error_info = {
                                                                          JwtError::INVALID_SIGNATURE,
                                                                          "Invalid token signature."
                                                                      };
                        break;
                    case qb::jwt::ValidationError::TOKEN_EXPIRED: error_info = {
                                                                      JwtError::TOKEN_EXPIRED, "Token has expired."
                                                                  };
                        break;
                    case qb::jwt::ValidationError::TOKEN_NOT_ACTIVE: error_info = {
                                                                         JwtError::TOKEN_NOT_ACTIVE,
                                                                         "Token is not yet active."
                                                                     };
                        break;
                    case qb::jwt::ValidationError::INVALID_ISSUER: error_info = {
                                                                       JwtError::INVALID_CLAIM, "Invalid issuer."
                                                                   };
                        break;
                    case qb::jwt::ValidationError::INVALID_AUDIENCE: error_info = {
                                                                         JwtError::INVALID_CLAIM, "Invalid audience."
                                                                     };
                        break;
                    case qb::jwt::ValidationError::INVALID_SUBJECT: error_info = {
                                                                        JwtError::INVALID_CLAIM, "Invalid subject."
                                                                    };
                        break;
                    case qb::jwt::ValidationError::CLAIM_MISMATCH:
                        error_info = {JwtError::INVALID_CLAIM, "A specific claim value mismatch occurred."};
                        break;
                    default: error_info = {JwtError::INVALID_TOKEN, "Unknown JWT validation error."};
                        break;
                }
                return std::nullopt;
            }
