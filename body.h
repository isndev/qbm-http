...
public:
    ...
    requires(sizeof...(Args) > 0 && !(sizeof...(Args) == 1 && (std::is_same_v<std::remove_cvref_t<Args>, Body> && ...))
     ...
     */
    static std::unique_ptr<qb::compression::decompress_provider> get_decompressor_from_header(const std::string &encoding);
...
public:
    ...
    [[nodiscard]] T
    as() const {
        static_assert(dependent_false<T>::value, "Body::as<T>() is not implemented for this T. "
