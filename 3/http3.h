...
struct stream_key_hash {
    std::size_t operator()(stream_key const& key) const noexcept {
...
public:
    ...
    [[nodiscard]] Handler& server() noexcept { return *_server; }
    [[nodiscard]] Handler const& server() const noexcept { return *_server; }
    ...
    [[nodiscard]] Router& router() noexcept { return _server->router(); }

    void bind_http3_stream(std::uint64_t connection_id, std::uint64_t stream_id) noexcept {
        _connection_id = connection_id;
        _stream_id = stream_id;
    }

    void bind_http3_request_method(qb::http::Method method) noexcept {
        _request_method = method;
    }

    void attach_context(std::shared_ptr<ContextType> context) noexcept {
    ...
    void cancel_context(std::string const& reason) noexcept {
        if (_context) {
    ...
    [[nodiscard]] bool has_active_context() const noexcept {
        return static_cast<bool>(_context);
    }

    auto& operator<<(qb::http::Response& response) {
...
private:
    ...
    [[nodiscard]] h3_connection *connection(std::uint64_t connection_id) noexcept {
        auto it = _connections.find(connection_id);
        return it == _connections.end() ? nullptr : it->second.get();
    }

    [[nodiscard]] h3_connection& ensure_connection(std::uint64_t connection_id) {
        auto it = _connections.find(connection_id);
    ...
    [[nodiscard]] bool has_sessions(std::uint64_t connection_id) const noexcept {
    ...
    [[nodiscard]] bool has_active_contexts(std::uint64_t connection_id) const noexcept {
    ...
    void maybe_finish_graceful_shutdown(std::uint64_t connection_id) {
    ...
    [[nodiscard]] std::shared_ptr<SessionType> ensure_session(std::uint64_t connection_id,
...
protected:
    ...
    void after_dispatch_events() override {
...
public:
    ...
    [[nodiscard]] Router& router() noexcept { return _router; }
    [[nodiscard]] Router const& router() const noexcept { return _router; }

    void set_max_body_size(std::size_t value) noexcept { _max_body_size = value; }
    [[nodiscard]] std::size_t max_http3_body_size() const noexcept { return _max_body_size; }

    bool listen(qb::io::uri const& uri,
    ...
    bool listen(std::string const& uri,
                std::filesystem::path const& cert_file,
                std::filesystem::path const& key_file) {
    ...
    void send_http3_stream_data(std::uint64_t connection_id, std::uint64_t stream_id,
                                std::string_view data, bool fin) {
        this->send_stream_data(connection_id, stream_id, data, fin);
    }

    void extend_http3_stream_credit(std::uint64_t connection_id, std::uint64_t stream_id,
                                    std::uint64_t bytes) {
        this->extend_stream_credit(connection_id, stream_id, bytes);
    }

    void reset_http3_stream(std::uint64_t connection_id, std::uint64_t stream_id,
                            std::uint64_t app_error_code) {
        this->reset_stream(connection_id, stream_id, app_error_code);
    }

    void stop_http3_stream(std::uint64_t connection_id, std::uint64_t stream_id,
                           std::uint64_t app_error_code) {
        this->stop_stream(connection_id, stream_id, app_error_code);
    }

    void close_http3_connection(std::uint64_t connection_id, std::uint64_t app_error_code,
                                std::string_view reason) {
        this->close_connection(connection_id, app_error_code, reason);
    }

    void on_http3_stream_acked(std::uint64_t, std::uint64_t) {}

    void on_http3_stream_closed(std::uint64_t connection_id, std::uint64_t stream_id,
                                std::uint64_t) {
    ...
    void on_http3_stream_output_drained(std::uint64_t connection_id,
                                        std::uint64_t stream_id) {
        stream_key key{connection_id, stream_id};
    ...
    void on_http3_shutdown(std::uint64_t, std::uint64_t) {
    }

    void on_http3_request(std::uint64_t connection_id, std::uint64_t stream_id,
    ...
    bool send_response(std::uint64_t connection_id, std::uint64_t stream_id,
