...
public:
    ...
    [[nodiscard]] bool is_connected() const noexcept { return _is_connected && _h3_ready; }
...
private:
    ...
    void fail_request(std::uint64_t stream_id, std::string const& error,
                      qb::http::status status = qb::http::status::BAD_GATEWAY);
    void fail_pending_request(std::uint64_t request_id, std::string const& error,
                              qb::http::status status = qb::http::status::BAD_GATEWAY);
